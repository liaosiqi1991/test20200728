...
Public Sub LoadDefaultModuleConfig()
    ...
    Dim i As Long
    Dim strReportModuleName As String
    
    If gblnUsePacsV2 Then
        strReportModuleName = "检查报告模块"
    Else
        strReportModuleName = "影像报告模块"
    End If
    
    Select Case mlngModule
        Case G_LNG_PACSSTATION_MODULE
            strModuleName = "影像图像模块;" & strReportModuleName & ";病历记录模块;费用记录模块;医嘱记录模块;电子病历模块;"
            
        Case G_LNG_VIDEOSTATION_MODULE
            strModuleName = "影像采集模块;" & strReportModuleName & ";病历记录模块;费用记录模块;医嘱记录模块;电子病历模块;"
            
        Case G_LNG_PATHOLSYS_NUM
            strModuleName = "影像采集模块;标本核收模块;病理取材模块;病理制片模块;病理特检模块;过程报告模块;" & strReportModuleName & ";病历记录模块;费用记录模块;医嘱记录模块;电子病历模块;"
        Case Else
            Exit Sub
    End Select
...
On Error Resume Next
    lngSelIndex = lstModule.ListIndex
    
    If InStr("影像图象模块,影像图像模块,影像采集模块,影像报告模块,检查报告模块", lstModule.list(lngSelIndex)) > 0 Then
         lstModule.Selected(lngSelIndex) = True
    End If
