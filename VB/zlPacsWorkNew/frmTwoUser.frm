...
Private Sub cmdChange_Click()
    ...
    '连接数据库
    If Not InitOraConn(strServerName, UCase(mstrUserIDNew), Trim(mstrPassWord), UCase(mstrUserIDNew) <> "SYS" And UCase(mstrUserIDNew) <> "SYSTEM") Then
...
Private Sub Form_Load()
    ...
    If mstrUserIDNew = "" Or intDBState = 1 Then   '最后的状态是统一，或者第一次进入
        ...
        txtUserID(0).Text = strUserIDHIS
        txtUserID(1).Text = strUserIDHIS
        
        txtUserID(1).SelStart = 0
        txtUserID(1).SelLength = 100
        
        txtUserID(0).BackColor = &HC0C0C0
        txtPassWord(0).BackColor = &HC0C0C0
    ...
    Else    '最后一次退出是交换状态
        If blnCnOracleIsNew = False Then         '报告人是通过导航台登陆的,检查医生是新登陆的
            ...
            txtUserID(0).SelStart = 0
            txtUserID(0).SelLength = 100
            ...
            txtUserID(1).BackColor = &H80000005
            txtPassWord(1).BackColor = &H80000005
        ...
        Else                                    '报告人是新登陆的，检查医生是通过导航台登陆的
            ...
            txtUserID(1).BackColor = &HC0C0C0
            txtPassWord(1).BackColor = &HC0C0C0
            
            txtUserID(1).Enabled = False
            txtPassWord(1).Enabled = False
            
            txtUserID(0).BackColor = &H80000005
...
Public Function InitOraConn(ByVal strServerName As String, ByVal strUserName As String, ByVal strUserPwd As String, Optional ByVal blnTran As Boolean = True) As Boolean
    Dim objRegister As Object
    Dim strError As String
    
On Error Resume Next
    Set objRegister = Interaction.GetObject("", "zlRegister.clsRegister")
    err.Clear
    
On Error GoTo ErrorHand
    If Not objRegister Is Nothing Then
        Set cnOracle = objRegister.GetConnection(strServerName, strUserName, strUserPwd, blnTran, , strError)
    End If
    
    If strError <> "" Then MsgBox strError
    
    InitOraConn = strError = ""
