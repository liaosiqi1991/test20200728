...
On Error GoTo errHand
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "提取NO", mlngAdviceId, mlngSendNo)
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "提取单据编号", mlngClinicID, CLng(Decode(mintSourceType, 1, 1, 2, 2, 1)))
    ...
    '新的方式
        ...
        If strResult = "0" Then
            MsgBoxD Me, "没有格式不能打印", vbInformation, gstrSysName
        Else
            strArry = Split(strResult, ",")
            For i = 0 To UBound(strArry)
                ReportOpen gcnOracle, glngSys, strBillNo, Me, "NO=" & strExseNo, "性质=" & intExseKind, "医嘱ID=" & mlngAdviceId, "ReportFormat=" & Val(strArry(i)), 2
            Next
        End If
    End If
    Exit Sub
...
Private Sub InitFaceScheme()
    ...
    mblnChoosePrintFormat = (Val(zlDatabase.GetPara("报到打印前选择格式", glngSys, mlngModul, 0)) = 1)
...
Dim strTmp As String
    ...
    gstrSQL = "SELECT I.名称 as 号别名称,H.病理号,H.检查类型,Nvl(E.姓名,A.姓名) 姓名,Nvl(E.性别,A.性别) 性别,Nvl(E.年龄,A.年龄) 年龄,B.出生日期,B.入院时间,B.费别,B.医疗付款方式,B.身份证号,B.民族,B.职业,Nvl(E.英文名,'') 英文名,E.身高,E.体重" & _
                    ...
                    ",Nvl(D.名称,'') AS 病人科室,A.病人科室ID,Decode(A.病人来源,2,J.住院号,K.门诊号) As 病人号,Decode(J.住院号,NULL,NULL,B.当前床号) As 床号,B.当前病区ID" & _
                    ...
                    ",F.记录性质 " & _
                " FROM 病人医嘱发送 F,病人医嘱记录 A, 病人信息 B,部门表 C,部门表 D,影像检查记录 E,影像检查项目 G, 病理检查信息 H ,病理号码规则 I,病案主页 J,病人挂号记录 K  " & _
                " Where K.no(+)= A.挂号单 And A.病人ID=J.病人ID(+) And A.主页ID=J.主页ID(+) And F.医嘱ID=[1] And F.发送号=[2] AND F.医嘱ID=A.ID And F.医嘱ID=H.医嘱ID(+) And H.号码规则ID=I.ID(+) " & _
                        " AND F.医嘱ID=E.医嘱ID(+) And F.发送号=E.发送号(+)  And A.病人ID=B.病人ID" & _
                        " And A.开嘱科室ID=C.ID And A.病人科室ID=D.ID And A.诊疗项目ID=G.诊疗项目ID(+)"
...
Private Sub PatiIdentify_FindPatiBefore(ByVal objCard As zlIDKind.Card, blnCard As Boolean, strShowText As String, objCardData As zlIDKind.PatiInfor, blnFindPatied As Boolean, blnCancel As Boolean)
    ...
    If Not objCardData Is Nothing Then objCardData.姓名 = PatiIdentify.Text
...
On Error GoTo errH
    ...
    If Not IsNull(rsTmp) Then
        ...
        If gblUsePacsQuery Then
            ...
            If objStudyInfo.strStuStateDesc = "已报到" Or _
               objStudyInfo.strStuStateDesc = "已检查" Or _
               (objStudyInfo.strStuStateDesc = "已登记" And mintEditMode = 2) Then
               cbo待处理人.Enabled = True
            Else
