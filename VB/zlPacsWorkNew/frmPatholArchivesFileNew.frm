...
On Error GoTo errH
    ...
    strSQL = "Zl_病理档案_更新文件档案(" & lngCurArchivesId & ",'" & txtArchivesName.Text & "','" & txtArchivesCode.Text & "','" & txtArchivesStudyArea.Text & "'," & _
                                cbxArchivesClass.ItemData(cbxArchivesClass.ListIndex) & "," & zlStr.To_Date(dtpArchivesStart.value) & "," & _
                                zlStr.To_Date(dtpArchivesEnd.value) & ",'" & cbxRoom.Text & "','" & cbxBox.Text & "','" & cbxDrawer.Text & "','" & txtPlace.Text & "','" & txtArchivesDescription.Text & "')"
...
On Error Resume Next
    ...
    Set mufgParentGrid = Nothing
    Set mrsArchivesClass = Nothing
End Sub
