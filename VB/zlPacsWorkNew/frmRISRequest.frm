...
Private mblnCanOverWrite As Boolean             '允许检查号重复
...
Private mblnUseReferencePatient As Boolean      '使用关联病人模式
...
Private mRequestInfo As TRequestInfo
...
Property Get RequestInfo() As TRequestInfo
...
Public Function ZlShowMe(frmParent As Form, ByVal strDefaultPatientType As String, ByVal blnBigFont As Boolean, Optional ByVal blnIsAllDepartment As Boolean = False, _
    ...
    mRequestInfo.dtTime = zlDatabase.Currentdate
    mRequestInfo.lngPatiID = mlngPatiId
    mRequestInfo.strType = mstrItemType
...
Private Function ShowPayMoneyWindow(ByVal str医嘱IDs As String) As Boolean
On Error GoTo errH
    ShowPayMoneyWindow = True
    If mobjSquareCard.zlSquareAffirm(Me, mlngModul, mstrPrivs, mlngPatiId, 0, False, , , str医嘱IDs) = False Then
        ...
        ShowPayMoneyWindow = False
    End If
    Exit Function
errH:
    MsgBoxD Me, "缴费流程发生异常：" & err.Description, vbOKOnly, "缴费失败"
    ShowPayMoneyWindow = False
End Function
...
Private Sub cmdOk_Click()
    ...
    If mintEditMode = 2 Or (mblnRegToCheck And mintEditMode = 0) Then
        ...
        gstrSQL = "select B.附加标志 From 病人医嘱记录 A, 病人挂号记录 B Where A.挂号单=B.No And A.ID=[1]"
        Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, "查询附加标志", mlngAdviceId)
        
        If rsTmp.RecordCount > 0 Then intTag = Val(nvl(rsTmp!附加标志, 0))
        
        lngResult = 0
        
        If intTag = 3 Then '新门诊病人
            lngResult = NewOut收费(mlngAdviceId)
        End If
            ...
            If int记录性质 = 1 _
                ...
                If Not ItemHaveCash(mintSourceType, False, mlngAdviceId, 0, lng发送号, str诊疗类别, str单据号, int记录性质, _
                    ...
                    If gbln执行前先结算 Then
                        ...
                        '急诊患者，并且启用 急诊报到时不执行费用 可以直接报到
                        If chk紧急.value = 1 And mblnEmergencyRequestNotExecuteMoney Then
                            If MsgBoxD(Me, "患者没有缴费，是否缴费后报到，选'否'将不缴费直接报到？", vbYesNo + vbDefaultButton2, "提示信息") = vbYes Then
                                If ShowPayMoneyWindow(str医嘱IDs) = False Then Exit Sub
                            Else
                                '急诊病人没缴费也可以报到
                            End If
                        Else
        ...
        If int记录性质 = 2 Then
            ...
            If curMoney <> 0 Then
                '记账报警
                If Not FinishBillingWarn(Me, ";" & GetPrivFunc(100, 1257) & ";", mlngPatiId, mlngPageID, Val(lblCash.tag), curMoney, str类别, str类别名) Then
                    Exit Sub
                End If
   ...
   End If
    ...
    If mlngGoOnReg = 1 And mintEditMode = 0 Then
        '先对上一个医嘱做预约，再继续登记
        If chkSchedule.Enabled = True And chkSchedule.value = 1 Then
            Call frmSchSchedule.ZlShowMe(mstrPrivs, mlngAdviceId, CStr(mlngCurDeptId), Me)
        End If
        
        RaiseEvent HaveRegist
        Call InitMvar(mblnNameColColorCfg) '初始化模块变量
        Call ClearFaceData
    ...
    Else
        ...
        '登记而且需要预约，则显示预约窗口库
        If mintEditMode = 0 And chkSchedule.Enabled = True And chkSchedule.value = 1 Then
            Call frmSchSchedule.ZlShowMe(mstrPrivs, mlngAdviceId, CStr(mlngCurDeptId), Me)
        End If
...
'lngRegType  '1-新登记、2-提取登记、3-复制登记
    ...
    '通过权限来控制病人部分基本信息是否能被修改
    blnIsForceModify = CheckPopedom(mstrPrivs, "强制修改住院门诊信息")
    ...
    Call sutSetTxtEnable(Txt电话, blnIsModifyInfo)
    Call sutSetTxtEnable(Txt邮编, blnIsModifyInfo)
    Call sutSetTxtEnable(Txt联系地址, blnIsModifyInfo)
    cbo民族.Enabled = blnIsModifyInfo
    cbo职业.Enabled = blnIsModifyInfo
    cbo婚姻.Enabled = blnIsModifyInfo
    
    '没有启用身份认证相关时，如果有强制修改门诊住院信息权限，则允许修改电话，联系地址等信息
    If blnIsModifyInfo = False And blnIsForceModify Then
        Call sutSetTxtEnable(Txt电话, blnIsForceModify)
        Call sutSetTxtEnable(Txt邮编, blnIsForceModify)
        Call sutSetTxtEnable(Txt联系地址, blnIsForceModify)
        cbo民族.Enabled = blnIsForceModify
        cbo职业.Enabled = blnIsForceModify
        cbo婚姻.Enabled = blnIsForceModify
    End If

    cbo费别.Enabled = (mintSourceType = 3)
    cbo付款方式.Enabled = (mintSourceType = 3)
    ...
    PatiIdentify.BackColor = Txt身份证号.BackColor
End Sub
...
Private Sub Form_Load()
    ...
    chkSchedule.value = IIf(GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\" & Me.Name, "登记后预约", 0) = 1, 1, 0)
    Call ClearFaceData
...
'功能：读取病人信息，并显示该病人存在的医嘱时间
    ...
    Select Case PatiIdentify.IDKindIDX
        ...
        Case PatiIdentify.GetKindIndex(IDKind_身份证号)
            ...
            Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "按姓名调用Zl_Custom_Patiids_Get", mlngModul, strCode, "")
        ...
        Case Else       '使用姓名的时候，经常直接刷卡，所以姓名和刷卡的放在一起处理
            If PatiIdentify.IDKindIDX = PatiIdentify.GetKindIndex(IDKind_姓名) Then
            '是姓名，优先使用Zl_Custom_Patiids_Get如果找到病人ID直接查找。
                                            strSQL = "Select Zl_Custom_Patiids_Get([1], [2], [3]) As ID from dual"
                Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "按姓名调用Zl_Custom_Patiids_Get", mlngModul, "", strCode)
...
'        mstrRegNo = strNO
    ...
    ElseIf mInputType = 10 Then '  ","连接的多个病人ID
            gstrSQL = "select 病人id,姓名,性别,年龄,出生日期,来源ID,主页ID,病人科室ID,当前科室ID,医生,门诊号,住院号,就诊卡号,就诊状态,卡验证码,当前床号,费别" & _
                        ",医疗付款方式,身份证号,民族,职业,婚姻状况,电话,邮编,地址,合同单位ID, 新病人" & _
                    " From(Select distinct A.病人id,A.姓名,A.性别,A.年龄,to_char(A.出生日期,'yyyy-mm-dd') 出生日期,Decode(A.当前科室id,Null,1,2) As 来源ID,A.主页ID," & _
                        "Decode(A.当前科室id,Null,Nvl(B.执行部门ID,0),A.当前科室id) As 病人科室ID,A.当前科室ID,Nvl(B.执行人,'') As 医生,A.门诊号,A.住院号,A.就诊卡号,decode(A.就诊状态,0,'正常',1,'等待就诊',2,'正在就诊','')as 就诊状态,A.卡验证码,A.当前床号," & _
                        "A.费别,A.医疗付款方式,A.身份证号,A.民族,A.职业,A.婚姻状况,Nvl(A.家庭电话,A.联系人电话) 电话," & _
                        "Nvl(A.家庭地址邮编,A.单位邮编) 邮编,Nvl(A.家庭地址,A.工作单位) 地址,A.合同单位ID, 0 as 新病人,B.登记时间" & _
                  " From 病人信息 A,病人挂号记录 B,Table(f_num2list([1])) C Where A.病人ID=C.Column_Value And A.病人ID=B.病人ID(+) And A.门诊号=B.门诊号(+) and '%'='%' " & _
                  " order by B.登记时间 desc) " '为免避一行也弹出窗口所以用%,%在ShowSQLSelect在限制
    End If
    ...
    If mInputType = 10 Then
        Set GetPatient = zlDatabase.ShowSQLSelect(Me, gstrSQL, 0, "查病人信息", False, "病人ID", "", False, False, True, objRect.Left, objRect.Top, PatiIdentify.Height, blnCancel, True, False, CStr(strSeek), strSeek, strNO)
    Else
...
Private Sub InitFaceScheme()
    ...
    If mblnNoshowAddons And Label29.Visible = True Then '不显示附加主诉，且附加主诉已经被显示，则关闭显示附加主诉
        ...
        cmdOk.Top = cmdOk.Top - 400: CmdCancle.Top = cmdOk.Top: chkRoom.Top = cmdOk.Top: chkSchedule.Top = cmdOk.Top: cmdPetitionCapture.Top = cmdOk.Top
    ...
    If mintCheckInMode = 1 Then     '精简模式
        ...
        cmdOk.Top = cmdOk.Top - frm其他信息.Height: CmdCancle.Top = cmdOk.Top: chkRoom.Top = cmdOk.Top: chkSchedule.Top = cmdOk.Top: cmdPetitionCapture.Top = cmdOk.Top
...
'返回： True--复制成功；False--复制信息不完整
    ...
    strSQL = "SELECT nvl(E.姓名,B.姓名) 姓名,nvl(E.性别,B.性别) 性别,nvl(E.年龄,B.年龄) 年龄,B.出生日期,B.费别,B.医疗付款方式,B.身份证号,B.民族,B.职业,Nvl(E.英文名,'') 英文名,E.身高,E.体重" & _
                    ...
                    ",NVL(D.名称,'') AS 病人科室,A.病人科室ID,A.挂号单,Decode(A.病人来源,2,H.住院号,I.门诊号) As 病人号,Decode(H.住院号,NULL,NULL,B.当前床号) As 床号" & _
                ...
                " FROM 病人医嘱发送 F,病人医嘱记录 A, 病人信息 B,部门表 C,部门表 D,影像检查记录 E,影像检查项目 G,病案主页 H,病人挂号记录 I " & _
                " Where I.no(+)= A.挂号单 And A.病人ID=H.病人ID(+) And A.主页ID=H.主页ID(+) And F.医嘱ID=[1] And F.发送号=[2] AND F.医嘱ID=A.ID" & _
...
Dim rsSchedule As ADODB.Recordset
    ...
    gstrSQL = "SELECT nvl(E.姓名,A.姓名) 姓名,nvl(E.性别,A.性别) 性别,nvl(E.年龄,A.年龄) 年龄,B.出生日期,B.入院时间,B.费别,B.医疗付款方式,B.身份证号,B.民族,B.职业,Nvl(E.英文名,'') 英文名,E.身高,E.体重" & _
                    ",B.婚姻状况,Nvl(B.家庭电话,B.联系人电话) 电话,Nvl(B.家庭地址邮编,B.单位邮编) 邮编,nvl(B.家庭地址,B.工作单位) 地址,B.合同单位ID,B.门诊号,B.就诊卡号,B.卡验证码" & _
                    ",NVL(D.名称,'') AS 病人科室,A.病人科室ID,Decode(A.病人来源,2,H.住院号,I.门诊号) As 病人号,Decode(H.住院号,NULL,NULL,B.当前床号) As 床号,B.当前病区ID" & _
...
'    Call SeekIndex(cbo开单科室, Nvl(rsTemp!科室简码), True, , True)
    ...
    If mstrRoom = "" Then
        strSQL = "select a.诊室名称 from 影像预约记录 a where a.医嘱id = [1]"
        Set rsSchedule = zlDatabase.OpenSQLRecord(strSQL, "提取预约信息", mlngAdviceId)
        If rsSchedule.EOF = False Then
            mstrRoom = nvl(rsSchedule!诊室名称)
        End If
    End If
    ...
    If mblnNameColColorCfg Then
        ...
        If rsTemp.RecordCount > 0 Then
            ...
            If Not mobjPublicPatient Is Nothing Then
                PatiIdentify.objTxtInput.ForeColor = mobjPublicPatient.GetPatiColor(nvl(rsTemp!病人类型))
            End If
        End If
    End If
...
'------------------------------------------------
    ...
    If Not rsTemp.EOF Then
        If nvl(rsTemp!参数值) <> "" Then
            ...
            ElseIf InStr(rsTemp!参数值, "身份证号") > 0 And Trim(Txt身份证号.Text) = "" And Txt身份证号.Enabled = True Then
    ...
    '检查出生日期
    If dtp出生日期.value > zlDatabase.Currentdate Then
        MsgBoxD Me, "当前出生日期大于今天的日期，请重新输入。", vbInformation, gstrSysName: DoEvents
        Me.dtp出生日期.SetFocus: Exit Function
    End If
...
'    End If
    ...
    If mblnUsePacsQueue Then
        ...
        If cboRoom.Text = "呼叫时指派" And cboRoom.Visible = True Then
            '当执行间为呼叫时指派时，需要判断当前检查项目是否设置了对应的执行分组，如果没有设置，则不允许指派
            If Not IsConfigStudyGroup(mlngClinicID) Then
                MsgBoxD Me, "该检查项目未设置对应的执行分组，不能在呼叫时指派执行间。", vbInformation, gstrSysName: DoEvents
                cboRoom.SetFocus
                Exit Function
            End If
        End If
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    End If
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\" & Me.Name, "登记后预约", chkSchedule.value
    ...
    If Not rsTmp Is Nothing Then
        If Not rsTmp.EOF Then
            If nvl(rsTmp!姓名) <> "新病人" Then
                ...
                Else
                    '根据当前日期计算年龄
                    LoadOldData ReCalcOld(dtp出生日期.value, cboAge, 0, dtp(0).value) & IIf(cboAge.Visible, cboAge.Text, ""), txt年龄, cboAge
                End If
...
Private Sub chkScheduleEnable()
'------------------------------------------------
'功能：是否显示预约框
'参数：
'返回：无
'------------------------------------------------
    Dim isDeptSchedule As Boolean
    Dim isOrderScheule As Boolean
    
    On Error GoTo err
    
    Call IsSchedule(mlngCurDeptId, 0, mlngClinicID, isDeptSchedule, isOrderScheule)
    
    chkSchedule.Visible = isDeptSchedule
    chkSchedule.Enabled = isOrderScheule
