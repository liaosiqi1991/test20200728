...
Private Function GetQueryWhereForAdvice(ByVal strSourceSql As String) As String
    ...
    '判断医嘱ID是否存在,如果不存在，则直接检索ID字段
    lngIndex = InStr(strSQL, "医嘱ID")
    If lngIndex <= 0 Then
        lngIndex = InStr(strSQL, "ID")
        If lngIndex > 0 Then lngIndex = lngIndex + Len("ID")
    Else
        lngIndex = lngIndex + Len("医嘱ID")
    End If
    ...
    'strSql的结果类似于 a.医嘱ID； a.Id as 医嘱ID；医嘱ID
...
Public Function GetQuerySqlForAdvice(ByVal strSourceSql As String) As String
    ...
    '单独将select...from语句提取出来，避免后面将select中的查询字段被替换为1
...
'获取参数在查询语句中对应使用的字段名称
    ...
    Else
        ...
        '如果包含between语句，则将between 的and后面的值直接设置为1
