...
Private Sub cmdSave_Click()
    If mblnNeedSaveSrv Then
        MsgBoxD Me, "上方服务列表中有变动且未保存，请先保存服务列表变动", vbInformation, gstrSysName
        Exit Sub
    End If
    ...
    Select Case TabList.Selected.Caption
        Case "图像接收服务"
            blnResult = mFrmImgSrv.SavePara
        Case "WorkList服务"
            blnResult = mfrmWorkList.SavePara
        Case "Q/R 查询服务"
            blnResult = mFrmQRSrv.SavePara
    End Select
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set mFrmImgSrv = Nothing
    Set mfrmWorkList = Nothing
    Set mFrmQRSrv = Nothing
End Sub
