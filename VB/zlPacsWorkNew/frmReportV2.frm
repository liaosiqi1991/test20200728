...
'    cbrToolBar.EnableDocking xtpFlagStretched
    ...
    With cbrToolBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_PacsReport_Save, "保存"): cbrControl.iconid = 3091: cbrControl.ToolTipText = "保存报告": cbrControl.BeginGroup = True
...
On Error GoTo errhandle
    '弹出报告内容保存提示
    If mblnIsLinkHelper = False Then mblnHasExitSave = mblnHasExitSave Or PromptSave
Exit Sub
errhandle:
...
On Error GoTo errH
    ...
    Exit Function
errH:
    If HintError(err, "GetSignVerifyType") = 1 Then Resume
End Function
...
'            mobjCapLinker.AddRepPacsHelper mobjStudyInfo.lngAdviceId, ucPacsHelper1
        ...
        Call InitCommandBar
        Call ucReportEditor1.Init(mObjNotify, lngModuleNo, lngDeptId, strPrivs, GetSignVerifyType())
    Else
...
Public Function GetReportId(ByVal lngAdviceId As Long) As Long
'获取检查对应的报告ID
    Dim strSQL As String
    Dim rsData As ADODB.Recordset
...
Public Sub SetReportTitle(objStudyInfo As clsStudyInfo)
    Me.Caption = "报告编辑    [" & objStudyInfo.strPatientName & "(" & objStudyInfo.strStudyNum & ")    " & objStudyInfo.strPatientAge & "    " & objStudyInfo.strPatientSex & "    " & objStudyInfo.strAdviceContext & "]" & "    《" & ucReportEditor1.EPRFmtName & "》"
End Sub
...
Public Sub zlRefresh(objStudyInfo As clsStudyInfo, _
    ...
    lngReportID = GetReportId(mobjStudyInfo.lngAdviceId)
    ...
    mlngFileFormatId = GetFileFormatId(mobjStudyInfo.lngAdviceId)
