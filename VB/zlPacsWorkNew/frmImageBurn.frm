    ...
    If rsCurImageData.RecordCount > 0 Then
        ...
        Do While Not rsCurImageData.EOF
            ...
            If mobjFile.FileExists(strTmpFile) = False Then
                ...
                struFtpTag = FtpTagInstance(nvl(rsCurImageData("Host1")), _
                                            nvl(rsCurImageData("User1")), _
                                            nvl(rsCurImageData("Pwd1")), _
                                            objFile.GetParentFolderName(nvl(rsCurImageData("Root1")) & rsCurImageData("URL")))
                                            
                If Trim(struFtpTag.Ip) = "" Then
                    struFtpTag = FtpTagInstance(nvl(rsCurImageData("Host2")), _
                                                nvl(rsCurImageData("User2")), _
                                                nvl(rsCurImageData("Pwd2")), _
                                                objFile.GetParentFolderName(nvl(rsCurImageData("Root2")) & rsCurImageData("URL")))
                End If
                
                lngResult = FtpDownload(struFtpTag, objFile.GetFileName(rsCurImageData("URL")), strTmpFile, False)
...
'创建DICOM目录
Private Function CreateDicomDir() As Boolean
        ...
        If Trim(strStudyUID) <> "" Then
            ...
            If ufgBurnData.GetRowCheck(i) = True And InStr(mstrReadyUID, strStudyUID & ";") <= 0 Then
                ...
                struFtpTag = FtpTagInstance(nvl(rsTemp!Host1), _
                                            nvl(rsTemp!User1), _
                                            nvl(rsTemp!Pwd1), _
                                            mobjFile.GetParentFolderName(nvl(rsTemp("Root1")) & nvl(rsTemp("URL"))))
                
                If Trim(struFtpTag.Ip) = "" Then
                    struFtpTag = FtpTagInstance(nvl(rsTemp!Host2), _
                                                nvl(rsTemp!User2), _
                                                nvl(rsTemp!Pwd2), _
                                                mobjFile.GetParentFolderName(nvl(rsTemp("Root2")) & nvl(rsTemp("URL"))))
                End If
...
'
                ...
                While Not rsTemp.EOF
                    ...
                    If mobjFile.FileExists(strTmpFile) = False Then
                        ...
                        lngResult = FtpDownload(struFtpTag, mobjFile.GetFileName(nvl(rsTemp("URL"))), strTmpFile, False)
...
On Error Resume Next
    ...
    Set mdsetDicomDir = Nothing
    Set mobjRegBurnFileList = Nothing
    Set mrsCustomQuery = Nothing
End Sub
...
On Error GoTo errHandle
    ...
    If CreateDicomDir = False Then
        stbThis.Panels.Item(4).Text = "待刻录文件大小："
        Exit Sub
    End If
