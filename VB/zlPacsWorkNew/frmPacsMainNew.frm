...
On Error GoTo errHand
    ...
    '新的方式
        ...
        If strResult = "0" Then
            MsgBoxD Me, "没有格式，不能打印", vbInformation, Me.Caption
        Else
...
'                    '方案不完全相同，重新加载新的默认方案  LSQ TODO  后续优化不需要重新加载方案的处理方式
                    ...
                    With cbrMenuBar.CommandBar
                        ...
                        Set objControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_Manage_QueryValidTime, "切换方案保留时间范围", "", 0, False)
...
Private Sub mfrmHistory_OnDoWork(ByVal LngAdvice As Long, ByVal strFuncName As String)
    Select Case strFuncName
        Case "观片"
            Call OpenViewer(1, mobjPacsCore, LngAdvice, False, Me, "", , mSysPar.blnLocalizerBackward, 0, "")
        Case "对比"
            Call OpenViewer(1, mobjPacsCore, LngAdvice, True, Me, "", , mSysPar.blnLocalizerBackward, 0, "")
        Case "查看报告"
            Call OpenReport
    End Select
...
Private Sub mobjPacsQueryWrap_OnEnableBackRefresh()
...
'            Control.Enabled = Not Control.Enabled
        ...
        Case conMenu_Manage_ReLoadPDF
            ...
            Control.Visible = mSysPar.strPDFFTPdevice <> "" And mSysPar.strPDFFTPdevice <> M_STR_PDF_NOPRINTER
...
'    mblnSetXWParam = IIf(InStr(GetPrivFunc(glngSys, G_LNG_XWPACSVIEW_MODULE), "PACS参数设置") > 0, True, False)
    ...
    Call WriteLog("InitForm -> Step 4：初始化自定义查询相关配置...")
    If mobjPacsQueryWrap Is Nothing Then
        ...
        Set objPar.cmdLocate = cmdLocate
    ...
    Call WriteLog("InitForm -> Step 5：初始化部门级参数...")
    Call InitModuleParameter '初始化模块级变量
    
    Call WriteLog("InitForm -> Step 6：初始化窗口菜单...")
    Call InitCommandBars

    Call WriteLog("InitForm -> Step 7：初始化界面布局...")
    Call initTabExtra
    Call InitFaceScheme
...
'Begin----------------------检查菜单--------------------------------------默认可见
    ...
    With cbrMenuBar.CommandBar
        ...
        Set cbrControl = CreateModuleMenu(.Controls, xtpControlPopup, conMenu_Manage_Request, "申请单", "申请单", 0, False)
            ...
            Set cbrPopControl = CreateModuleMenu(cbrControl.CommandBar.Controls, xtpControlButton, conMenu_Manage_CheckList, "查看电子申请", "查看电子申请单", 3564, False)
        
        Set cbrControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_Check_ViewLink, "查看关联检查", "", 102, False): cbrControl.ToolTipText = "查看关联检查"
        ...
        If Not cbrControl Is Nothing Then
            ...
            Set cbrPopControl = CreateModuleMenu(cbrControl.CommandBar.Controls, xtpControlButton, conMenu_Manage_Undone, "取消完成", "", 219, False)
        ...
        If mlngModule <> G_LNG_PACSSTATION_MODULE Then
            Set cbrControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_Tool_Analyse, "高级处理", "", 0, True): cbrControl.ToolTipText = "高级图像处理"
        End If
...
'Begin----------------------自定义查询菜单--------------------------------------默认可见
    ...
    With cbrMenuBar.CommandBar
        ...
        Set cbrControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_Manage_QueryValidTime, "切换方案保留时间范围", "", 0, False)
        cbrControl.Checked = mSysPar.blnQuickTabDisplayScheme
        cbrControl.CloseSubMenuOnClick = False
    End With
...
On Error GoTo errhandle
    ...
    If mSysPar.strPDFFTPdevice <> "" And mrtReportType = 电子病历编辑器 Then
...
On Error GoTo errhandle
    ...
    '报告审核后
        '清空待处理人
        Call Menu_Manage_SendAudit("")
        If mSysPar.strPDFFTPdevice <> "" Then blnDoPDF = True
...
On Error GoTo errhandle
    ...
    If Not mobjWork_His Is Nothing Then
        ...
        Call mobjWork_His.zlUpdateOtherInf(mobjCurStudyInfo.lngPatId, mobjCurStudyInfo.lngUnit, mobjCurStudyInfo.lngPatDept, mobjCurStudyInfo.lngPageID, _
            mobjCurStudyInfo.intState, mobjCurStudyInfo.strRegNo, mblnIsHistory, mobjCurStudyInfo.blnIsInsidePatient)
    End If
