...
Private marrImage() As DicomImage
Private mblnAddReportImage As Boolean
...
Public Sub zlRefresh(ByVal lngAdviceId As Long, FileID As Long, _
    ...
    mstrAfterImgPath = FormatFilePath(GetAppRootPath & "\Apply\TmpAfterImage\")
...
On Error GoTo errHandle
    ...
    While Not rsImageDatas.EOF
        If mobjFile.FileExists(rsImageDatas!路径) Then
            Call mobjFile.DeleteFile(rsImageDatas!路径)
        End If
...
On Error GoTo errHandle
    ...
    If rsImageDatas Is Nothing Then Exit Function
    If rsImageDatas.RecordCount <= 0 Then
        Call MsgBoxD(Me, "找不到待关联的图像信息。", vbInformation, Me.Caption)
        Exit Function
    End If
    ...
    If Trim(nvl(rsTmp!检查UID)) = "" Or Trim(nvl(rsTmp!接收日期)) = "" Then
        ...
        Call GetFtpDeviceWithStudy(mlngModule, mlngAdviceId, strNewStudyUID, ftpInfo)
        
        If Trim(ftpInfo.strFtpIp) = "" Then
            Call MsgBoxD(Me, "不能取得有效的存储设备，请检查存储设备配置。", vbInformation, Me.Caption)
            Exit Function
        End If
        
        '更新存储设备信息
        strSQL = "Zl_影像检查_更新设备(" & mlngAdviceId & ",'" & strNewStudyUID & "','" & ftpInfo.strDeviceId & "'," & _
    ...
    '连接FTP
    struFtpTag = FtpTagInstance(ftpInfo.strFtpIp, ftpInfo.strFTPUser, ftpInfo.strFTPPwd, ftpInfo.strFtpVirtualURL)
    
    '上传图像文件
    rsImageDatas.MoveFirst
    While Not rsImageDatas.EOF
        lngResult = FtpUpload(struFtpTag, mobjFile.GetFileName(rsImageDatas!路径), rsImageDatas!路径)
        If lngResult = frAbort Then
            '清除之前上传的文件
            Call CancelImageMove(struFtpTag, objMoveList)
            Exit Function
        End If
        
        Call objMoveList.Add(mobjFile.GetFileName(rsImageDatas!路径))
...
On Error GoTo errHandle
    ...
    If rsImageDatas Is Nothing Then Exit Function
    If rsImageDatas.RecordCount <= 0 Then
        Call MsgBoxD(Me, "找不到需要取消关联的图像信息。", vbInformation, Me.Caption)
        Exit Function
    End If
    
    '撤销图像关联
    strNewStudyUID = rsImageDatas!检查UID
    ...
    While Not rsImageDatas.EOF
        ...
        lngResult = FtpDownload(struFtpTag, rsImageDatas!图像UID, strDestPath & rsImageDatas!图像UID, False)
    ...
    rsImageDatas.MoveFirst
    While Not rsImageDatas.EOF
        Call FtpDelete(struFtpTag, nvl(rsImageDatas!图像UID), False, False)
        
        If mlngModule <> 1290 Then
            Call FtpDelete(struFtpTag, nvl(rsImageDatas!图像UID) & ".jpg", False, False)
        End If
...
'撤销图像的移动
Private Sub CancelImageMove(ByRef struFtpTag As TFtpConTag, objMoveList As Collection)
    Dim i As Long
    ...
    For i = 1 To objMoveList.Count
        FtpDelete struFtpTag, objMoveList.Item(i), False, False
    Next
...
Public Function DcmAddImage(dcmImage As DicomImage, SelViewerIndex As Integer) As Boolean
'把当前图形添加到图象框中
On Error GoTo errH
    Dim i As Integer
    DcmAddImage = True
    ...
    '如果还没有创建图像框，则退出
    '处理136121 过程修改为函数，取消对ref 入参的修改，改为过程结束后修改。
    If dcmReportImage.Count = 1 Then
        DcmAddImage = False
        Exit Function
    End If
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set mSelMiniImg = Nothing
    Set mSelReportImg = Nothing
    Set pobjMarks = Nothing
    Set mobjPacsCapture = Nothing
    Set mobjImageProcess = Nothing
    Set mdcmGlobal = Nothing
    Set mrsImageCache = Nothing
    Set mdcmUID = Nothing
    Set mobjFile = Nothing
    
    ReDim mobjImgCTables(0)
    Set mobjImgCTables(0) = Nothing
    ReDim marrImage(0)
    Set marrImage(0) = Nothing
...
Private Sub LoadReportImages()
    ...
    If UBound(marrImage) > 0 Then
        For i = 1 To UBound(marrImage)
            '保持136121之前的逻辑
            If Not DcmAddImage(marrImage(i), IIf(mselReportImgIndex = 0, 1, mselReportImgIndex)) Then
                ReDim Preserve marrImage(UBound(marrImage) + 1)
                Set marrImage(UBound(marrImage)) = marrImage(i)
            End If
            mblnAddReportImage = True
        Next
        ReDim Preserve marrImage(0)
    End If
...
'-----------------------------------------------
    ...
    If blnLoadImages = False Then
        ...
        If Not mblnAddReportImage Then
...
Public Sub ReportImageAdd(ByVal strInstanceUID As String, ByVal dcmImage As DicomImage)
    '标记图像为报告图，并自动加入报告中
    Dim dcm As DicomImage
    Dim i As Integer

    For i = 1 To ucMiniImageViewer.ImgViewer.Images.Count
        If ucMiniImageViewer.ImgViewer.Images(i).InstanceUID = strInstanceUID Then
            Set dcm = ucMiniImageViewer.ImgViewer.Images(i)
            If dcm.tag.ReportImage = "" Then
                '设置“报告图”计数器
                dcm.tag.ReportImage = 1
                '画“报告图”标记
                Call ucMiniImageViewer.DrawReportImgTag(dcm)
                '添加报告图
