...
Function XWDownLoadImage(strStudyUID As String) As Long
'从云平台下载图像
'strStudyUID:检查UID
    ...
    strSQL = "P_OEM_DOWNLOADIMG_RIS('" & strStudyUID & "')"
...
'--------------------------------------------
    ...
    If Not CheckPopedom(";" & GetPrivFunc(glngSys, G_LNG_XWPACSVIEW_MODULE) & ";", "基本") Then
        MsgBox "用户没有授权，无法打开观片", vbInformation, gstrSysName
        Exit Function
    End If
    ...
    If CheckPopedom(strXwPrivs, "PACS保存重建图像") Then
        lngFunction = lngFunction Or &H2
    End If
    
    If CheckPopedom(strXwPrivs, "PACS胶片打印") Then
        lngFunction = lngFunction Or &H200
    End If
    
    If CheckPopedom(strXwPrivs, "PACS图像导出") Then
        lngFunction = lngFunction Or &H40000
    End If
    
    If CheckPopedom(strXwPrivs, "PACS GSPS保存") Then
    ...
    If XWADViewerOpen <> 0 Then
        MsgBox "ADViewer打开错误，返回的信息是：" & XWADViewerOpen, vbInformation, gstrSysName
    End If
...
'--------------------------------------------
    ...
    If strServerName = "" Then
        MsgBox "未找到数据库服务器配置信息，请设置。", vbInformation, gstrSysName
        Exit Sub
    End If
    ...
    If err <> 0 Or cnTest Is Nothing Then
        '数据库连接错误
        MsgBox "数据库连接失败。" & vbCrLf & vbCrLf & "错误代码是：" & err.Number & "；错误描述是： " & err.Description, vbInformation, gstrSysName
        Exit Sub
    End If
    
    MsgBox "数据库连接成功。", vbInformation, gstrSysName
    Exit Sub
err:
    If ErrCenter() = 1 Then
        Resume
...
'--------------------------------------------
    ...
    If strDataSource = "" Then
        MsgBox "未找到SQLSERVER数据库服务器，请在“影像RIS工作站”的PACS参数中设置。", vbInformation, gstrSysName
        XWDBServerOpen = 1
        Exit Function
    End If
    ...
    If err <> 0 Or gcnXWDBServer Is Nothing Then
        '数据库连接错误
        MsgBox "DBServer数据库连接错误，可能会导致部分图像无法查看。" & vbCrLf & vbCrLf & "错误代码是：" & err.Number & "；错误描述是： " & err.Description, vbInformation, gstrSysName
    End If
...
Public Function XWSaveReportImagesV2(objParent As Object, ByVal lngOrderID As Long) As String
    Dim dcmImage As New DicomImage
    Dim strInstanceUID As String
    Dim strFileName As String
    Dim strSQL As String
    Dim rsTemp As ADODB.Recordset
    Dim objBgImgInfo As New clsBgImgInfo
    ...
    Dim ftpTransState As ftpResult
    
    XWSaveReportImagesV2 = ""
    
    strInstanceUID = CreateUID
    
    strFileName = strInstanceUID
    strLocalFile = FormatFilePath(objBgImgInfo.FilePath & "\" & strFileName)
    
    
    strResult = ResetStorageDevice(lngOrderID, objBgImgInfo)
    If Len(strResult) > 0 Then
        If gblnXWLog = True Then
            Call WriteCommLog("XWSaveReportImagesV2", "XW接口", "提取存储设备信息失败 [" & strResult & "], 尝试获取新的存储设备，医嘱ID = " & lngOrderID & "，执行科室ID= " & glngXWDeptID)
        End If
    
        Exit Function
    End If
    
    If Len(objBgImgInfo.StudyUID) <= 0 Then
        If gblnXWLog = True Then
            Call WriteCommLog("XWSaveReportImagesV2", "XW接口", "对应检查UID信息无效，医嘱ID = " & lngOrderID & "，执行科室ID= " & glngXWDeptID)
        End If
        
        Exit Function
    End If
    
    If Len(objBgImgInfo.FtpIp) <= 0 Then
        strResult = GetNewStorageDevice(lngOrderID, objBgImgInfo.StudyUID, objBgImgInfo.RecFmtDate, objBgImgInfo)
        If Len(strResult) > 0 Then
            If gblnXWLog = True Then
                Call WriteCommLog("XWSaveReportImagesV2", "XW接口", "获取新的存储设备失败[" & strResult & "], 医嘱ID = " & lngOrderID & "，执行科室ID= " & glngXWDeptID)
            End If
    ...
    objBgImgInfo.Filename = strFileName & ".jpg"
    
    If DirExists(objBgImgInfo.FilePath) = False Then Call MkLocalDir(objBgImgInfo.FilePath)

    dcmImage.Paste
    dcmImage.InstanceUID = strInstanceUID
    ...
    Call dcmImage.PrinterImage(8, iPlane, True, dblZoom, 0, dcmImage.SizeX, 0, dcmImage.SizeY).FileExport(strLocalFile, "JPG")

    If gblnXWLog = True Then
        Call WriteCommLog("XWSaveReportImagesV2", "XW接口", "保存本地图像，图像文件名为：" & strLocalFile)
    End If
 
    
    
    If gblnXWLog = True Then
        Call WriteCommLog("XWSaveReportImagesV2", "XW接口", "上传图像 " & strLocalFile & " 到FTP，FTP IP地址= " & objBgImgInfo.FtpIp & "，FTP子目录=" & objBgImgInfo.FtpVirtualPath & "，图像文件名为：" & strFileName)
    End If

    ftpConTag = FtpTagInstance(objBgImgInfo.FtpIp, objBgImgInfo.FtpUser, objBgImgInfo.FtpPwd, objBgImgInfo.FtpVirtualPath)
    
    ftpTransState = FtpUpload(ftpConTag, objBgImgInfo.FtpFile, strLocalFile)
    If ftpTransState <> frNormal Then
        If gblnXWLog = True Then
            Call WriteCommLog("XWSaveReportImagesV2", "XW接口", "图像 " & strLocalFile & " 上传到FTP失败，FTP IP地址= " & objBgImgInfo.FtpIp & "，FTP子目录=" & objBgImgInfo.FtpVirtualPath & "，图像文件名为：" & strFileName)
        End If
    ...
    If gblnXWLog = True Then
        Call WriteCommLog("XWSaveReportImagesV2", "XW接口", "写入报告图像信息到数据库.")
    End If
    
    '修改数据库，增加报告图
    strSQL = "ZL_影像检查报告_ADD('" & objBgImgInfo.StudyUID & "','" & strFileName & "')"
    zlDatabase.ExecuteProcedure strSQL, "保存报告图像"

    If gblnXWLog = True Then
        Call WriteCommLog("XWSaveReportImagesV2", "XW接口", "执行报告图保存过程：" & strSQL)
    End If
    

    strSQL = IIf(Trim(gstrOracleOwner) <> "", gstrOracleOwner & ".", "") & "b_XINWANGInterface.PacsSetFTPDeviceNo(" & lngOrderID & ",'" & objBgImgInfo.DeviceNo & "')"
    zlDatabase.ExecuteProcedure strSQL, "保存报告图像的FTP设备号"
    
    If gblnXWLog = True Then
        Call WriteCommLog("XWSaveReportImagesV2", "XW接口", "写入保存报告图的设备号，执行存储过程：" & strSQL)
    End If
    
    XWSaveReportImagesV2 = strLocalFile

End Function

Public Sub XWSaveReportImages(objParent As Object, ByVal lngOrderID As Long)
'------------------------------------------------
'功能：将图像从剪贴板保存成报告图
'参数： lngOrderID -- 医嘱ID
'返回：
'------------------------------------------------
    ...
    Dim frmReportImg1 As frmReportImage
    Dim i As Integer
    Dim rtReportType As ReportType
    Dim j As Integer
    Dim blnAddImg As Boolean
    ...
    Call GetDeptStorageDevice(objParent, lngOrderID, strStudyUID, glngXWDeptID, G_LNG_PACSSTATION_MODULE, gblnXWMoved, strDeviceNo, _
                    ...
                    If Not mobjReportImageInfo Is Nothing Then
                        For i = 0 To mobjReportImageInfo.Count - 1
                            ...
                            If Not frmReportImg Is Nothing Then
                                '报告窗体的医嘱ID跟报告图的医嘱ID相同，才添加
                                If frmReportImg.mlngAdviceId = lngOrderID Then
                                    '如果是弹出窗口，直接增加图像
                                    If frmReportImg.mblnSingleWindow = True Then
                                    ...
                                    Else
                                        '如果是嵌入式窗口，同时有相同医嘱ID的弹出窗口，则不增加
                                        blnAddImg = True
                                        For j = 0 To mobjReportImageInfo.Count - 1
                                            Set frmReportImg1 = mobjReportImageInfo(mobjReportImageInfo.Keys(j))
                                            If Not frmReportImg1 Is Nothing Then
                                                If frmReportImg1.mlngAdviceId = frmReportImg.mlngAdviceId And frmReportImg1.mblnSingleWindow = True Then
                                                    blnAddImg = False
                                                    Exit For
                                                End If
                                            End If
                                        Next j
                                        If blnAddImg = True Then
                                            Call frmReportImg.LoadMiniImages
                                            Call frmReportImg.DcmAddXWImage(dcmImage.PrinterImage(8, iPlane, True, dblZoom, 0, dcmImage.SizeX, 0, dcmImage.SizeY))
                                        End If
                                    End If
