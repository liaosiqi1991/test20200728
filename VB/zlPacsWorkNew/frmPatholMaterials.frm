...
Begin VB.Form frmPatholMaterials 
   ...
   Begin MSComctlLib.ImageList imgTbrS 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage9 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmPatholMaterials.frx":63EE
            Key             =   ""
         EndProperty
      EndProperty
   End
   Begin VB.PictureBox picPane1 
      ...
      Begin TabDlg.SSTab tsFilter 
         ...
         WordWrap        =   0   'False
         TabCaption(0)   =   "巨检描述(&D)"
         TabPicture(0)   =   "frmPatholMaterials.frx":6B68
         Tab(0).ControlEnabled=   0   'False
         Tab(0).ControlCount=   0
         TabCaption(1)   =   "脱钙管理(&C)"
         TabPicture(1)   =   "frmPatholMaterials.frx":6B84
...
On Error GoTo ErrorHand
    ...
    Select Case Control.ID
        ...
        Case conMenu_PatholMaterial_Cancel
            '材块回退
            Call CancelSelectionMaterial
...
On Error GoTo ErrorHand
    ...
    Select Case Control.ID
        ...
        Case conMenu_PatholMaterial_Get, conMenu_PatholMaterial_Del, conMenu_PatholMaterial_Save, _
            conMenu_PatholMaterial_Sure, conMenu_PatholMaterial_RequestView, conMenu_PatholMaterial_Cancel
...
'创建影像记录对应的菜单
    ...
    If Not HasMenu(objMenuBar, conMenu_PatholMaterial) Then
        ...
        With cbrMenuBar.CommandBar
            ...
            Set cbrControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_PatholMaterial_Sure, "确认取材(&U)", "", 0, True)
            Set cbrControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_PatholMaterial_Cancel, "材块回退(&R)", "", 0, True)
...
'根据菜单ID执行对应功能
    ...
    Select Case lngMenuId
        ...
        Case conMenu_PatholMaterial_Cancel         '材块回退
            Call CancelSelectionMaterial
...
Private Sub CancelSelectionMaterial()
On Error GoTo errH
    Dim strSQL As String
    Dim Row As Long
    Dim ID As String
    ...
    If Not ufgData.IsSelectionRow Then
        Call MsgBoxD(Me, "请选择需要回退的材块记录。", vbOKOnly, Me.Caption)
        Exit Sub
    End If
    ...
    If Not ufgData.IsEmptyKey(ufgData.SelectionRow) Then
        If Not CheckAllowUpdate(ufgData.KeyValue(ufgData.SelectionRow)) Then
            Call MsgBoxD(Me, "该材块记录已执行制片处理，请先回退制片流程。", vbOKOnly, Me.Caption)
            Exit Sub
        End If
    End If
    ...
    If Not ufgData.IsEmptyKey(ufgData.SelectionRow) Then
        If CheckRequisitionSlices(ufgData.KeyValue(ufgData.SelectionRow)) Then
            Call MsgBoxD(Me, "该材块记录已申请制片处理，请先回退制片流程及撤销申请。", vbOKOnly, Me.Caption)
            Exit Sub
        End If
    End If
    ...
    If Not ufgData.IsEmptyKey(ufgData.SelectionRow) Then
        If CheckRequisitionSpeExam(ufgData.KeyValue(ufgData.SelectionRow)) Then
            Call MsgBoxD(Me, "该材块记录已申请特检处理，请先回退特检流程及撤销申请。", vbOKOnly, Me.Caption)
            Exit Sub
        End If
    End If
    
    If MsgBoxD(Me, "确认要回退选择的材块数据吗？", vbYesNo, Me.Caption) = vbNo Then Exit Sub
    
    Row = ufgData.SelectionRow

    ID = ufgData.Text(Row, gstrMaterial_材块ID)
    If Trim(ID) = "" Then
        Call MsgBoxD(Me, "请选择需要回退的项目。")
        Exit Sub
    End If
    
    strSQL = "Zl_病理取材_退回(" & ID & ")"
    Call zlDatabase.ExecuteProcedure(strSQL, Me.Caption)
    
    Call ufgData.SyncText(Row, gstrMaterial_确认状态, "未确认", True)
    Exit Sub
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
'删除选中的材块
    ...
    If Not ufgData.IsEmptyKey(ufgData.SelectionRow) Then
        '判断材块是否已进行确认
        If CheckIsSure(ufgData.KeyValue(ufgData.SelectionRow)) Then
            Call MsgBoxD(Me, "该材块已进行确认不能进行删除，请先进行回退处理。", vbOKOnly, Me.Caption)
            Exit Sub
        End If
...
Private Function CheckIsSure(ByVal strMaterialId As String) As Boolean
'检查材块是否已进行确认
On Error GoTo errH
    ...
    strSQL = "select 材块ID from 病理取材信息 where  材块ID=[1] and 确认状态<>0"
'    If mblnMoved Then strSql = GetMovedDataSql(strSql)
    
    Set rsData = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strMaterialId)
    
    If rsData.RecordCount > 0 Then CheckIsSure = True
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
On Error GoTo errH
    ...
    '只有撤销申请后，才能删除对应的材块信息(单独进行的制片申请，制片方式不为0)
    strSQL = "select 申请ID from 病理制片信息 where 材块ID=[1] and 申请ID is not null and 制片方式 <> 0"
'    If mblnMoved Then strSql = GetMovedDataSql(strSql)
...
'确认当前取材信息
    ...
    '非取材阶段，不能进行确认
    If mrecStudy.lngMaterialStep <> TExecuteStep.NeedDo And mrecStudy.lngSlicesStep > 1 Then
        Call MsgBoxD(Me, "尚未进入取材阶段，或者已经进入制片流程，不能进行取材确认操作。", vbOKOnly, Me.Caption)
        Exit Sub
    End If
...
On Error GoTo errH
    ...
    With cbrToolBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_PatholMaterial_Cancel, "材块回退"): cbrControl.IconId = 5019: cbrControl.ToolTipText = "材块回退"
        cbrControl.BeginGroup = True
    End With
    Exit Sub
errH:
