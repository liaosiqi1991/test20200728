...
Private Sub Menu_Edit_Delete()
    ...
    '先检查是否被设置成预约设备，如果是，则提示用户，禁止删除
    strSQL = "select count(id) as 总数 from 影像预约设备 a where a.影像设备号=[1]"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "查找影像预约设备", Mid(Me.lvwItem.SelectedItem.Key, 2))
    If rsTemp!总数 > 0 Then
        Call MsgBoxD(Me, "设备“" & Me.lvwItem.SelectedItem.SubItems(2) & "”已经被设置成预约设备，请先删除预约设备后，再删除“" & Me.lvwItem.SelectedItem.SubItems(2) & "”。", vbOKOnly, gstrSysName)
        Exit Sub
    End If
    
    strSQL = "zl_影像设备目录_Delete('" & Mid(Me.lvwItem.SelectedItem.Key, 2) & "')"
...
'显示数据
    If mblnBeginchange And mblnChanged Then '开始修改或新增后
        If MsgBoxD(Me, "当前变更尚未保存，切换到其他设备会导致修改丢失，是否仍然切换？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
            TxtName.SetFocus
            Exit Sub
        End If
    End If
