...
Private Sub rtfAdvice_KeyPress(KeyAscii As Integer)
    If Len(rtfAdvice) >= 255 Then KeyAscii = 0
End Sub
...
'    End If
    ...
    If Not CheckWords Then Exit Sub
...
Private Function CheckWords() As Boolean
    Dim i As Long
    Dim strJudge As String
    Dim strImprove As String
    Dim strRemark As String
    
    For i = 1 To ufgData.GridRows - 1
        If LenB(StrConv(ufgData.Text(i, gstrPatholQuality_评价意见), vbFromUnicode)) > 255 Then
            strJudge = strJudge & IIf(Len(strJudge) > 0, "、", "") & i
        End If
        
        If LenB(StrConv(ufgData.Text(i, gstrPatholQuality_改进方法), vbFromUnicode)) > 255 Then
            strImprove = strImprove & IIf(Len(strImprove) > 0, "、", "") & i

        End If
        
        If LenB(StrConv(ufgData.Text(i, gstrPatholQuality_备注), vbFromUnicode)) > 1024 Then
            strRemark = strRemark & IIf(Len(strRemark) > 0, "、", "") & i
        End If
    Next i
    
    If Len(strJudge) > 0 Then
        strJudge = "第" & strJudge & "行【评价意见】的字符长度超过了255，"
    End If
    
    If Len(strImprove) > 0 Then
        strImprove = "第" & strImprove & "行【改进方法】的字符长度超过了255，"
    End If
    
    If Len(strRemark) > 0 Then
        strRemark = "第" & strRemark & "行【备注】的字符长度超过了1024，"
    End If
    
    If Len(strJudge) > 0 Or Len(strImprove) > 0 Or Len(strRemark) Then
        Call MsgBoxD(Me, strJudge & strImprove & strRemark & "无法保存。", vbOKOnly, Me.Caption)
        Exit Function
    End If
    
    If LenB(StrConv(rtfAdvice.Text, vbFromUnicode)) > 255 Then
        Call MsgBoxD(Me, "【综合意见】的字符长度超过了255，无法保存。", vbOKOnly, Me.Caption)
        rtfAdvice.SetFocus
        Exit Function
    End If
