...
Private mblHaveHistory As Long           '历史检查次数
Private mcboStudyHistory As ComboBox
Private mobjStudyInfo As clsStudyInfo
Private mlngStudyHistoryCount As Long
    ...
    Select Case Control.ID
        Case conMenu_Img_Look       '观片，当前检查有图像，或者是患者有历史检查，则可以观片
            If gblUsePacsQuery Then
                Control.Enabled = (mstrStudyUID <> "") Or mintViewHistoryImageDays > 0
            Else
        ...
        Case conMenu_Img_Contrast   '观片对比，只有中联PACS才显示
            If gblUsePacsQuery Then
                Control.Enabled = (mstrStudyUID <> "") Or mblHaveHistory
            Else
                Control.Enabled = (mstrStudyUID <> "") Or (mlngStudyHistoryCount > 1)
            End If
...
Public Sub zlUpdateOtherInf(blHaveHistory As Boolean, blnIsTechincalSure As Boolean, strTechincalDoctor As String, Optional cboHistory As Object = Nothing)
    '技师是否确认
    ...
    If Not cboHistory Is Nothing Then
        Set mcboStudyHistory = cboHistory
        mlngStudyHistoryCount = mcboStudyHistory.ListCount
    Else
        '自动打开历史图像
        mblHaveHistory = blHaveHistory
    End If
...
Private Sub OpenLatestImageOld()
