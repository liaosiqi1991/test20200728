...
Private Enum cboName
    ...
    年龄单位 = 1
End Enum
Public Sub ShowMe(ByVal lng病人ID As Long, ByVal lng主页ID As Long, Owner As Object)
    mlng病人ID = lng病人ID
    mlng主页ID = lng主页ID
    Me.Show 1, Owner
End Sub
...
'功能：读取指定病人信息,并显示在界面上
    ...
    strSQL = "Select a.病人id, a.姓名, a.性别, a.年龄, a.门诊号, a.费别, a.医疗付款方式, a.险类, a.区域, a.国籍, a.民族, a.学历," & vbNewLine & _
            ...
            "            To_char(b.入院日期,'yyyy-mm-dd hh24:mi:ss') 入院日期, To_char(b.出院日期,'yyyy-mm-dd hh24:mi:ss') 出院日期,A.其他证件,Nvl(B.病人类型,Decode(B.险类,Null,'普通病人','医保病人')) 病人类型  , A.手机号 " & vbNewLine & _
    ...
    Else
        strTxt = "病人ID=0,姓名=1,性别=2,年龄=3,门诊号=4,费别=5,医疗付款方式=6,医保号=7,险类=8,区域=9,国籍=10,民族=11,学历=12,婚姻状况=13,职业=14," & _
                ...
                " 出院中医诊断=54,登记时间=63,就诊卡号=64,其他证件=65,病人类型=66,手机号=67"
    End If
    ...
    If lng主页ID > 0 Then
        ...
        strSQL = "Select 诊断类型,疾病ID,诊断描述 From 病人诊断记录 Where nvl(录入次序,'01')='01' and nvl(编码类别,'E')='E'and 诊断次序=1 And" _
                   & " ( (记录来源=2 and 诊断类型 in(1,11,3,13)) or ( 记录来源=3 and 诊断类型 in(2,12)  ) )And 病人ID=[1] And 主页ID=[2]"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng病人ID, lng主页ID)
        
        If rsTmp.EOF Then
            strSQL = "Select 诊断类型,疾病ID,诊断描述 From 病人诊断记录 Where 诊断次序=1 And ( (记录来源=2 and 诊断类型 in(1,11,3,13)) or ( 记录来源=3 and 诊断类型 in(2,12)  ) ) And 病人ID=[1] And 主页ID=[2]"
            Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng病人ID, lng主页ID)
        End If
