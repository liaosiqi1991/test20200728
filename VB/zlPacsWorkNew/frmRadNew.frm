...
On Error GoTo errHandle
    ...
    For Each mobjItem In Me.lvwItem.ListItems
        If mobjItem.Checked Then
            gstrSQL = "zl_影像检查项目_Insert(" & Mid(mobjItem.Key, 2) & "," & strDescribe & ")"
        
            Call zlDatabase.ExecuteProcedure(gstrSQL, "保存影像检查项目")
            RaiseEvent OnRadNew(Val(Mid(mobjItem.Key, 2)))
        End If
    Next
...
Private Sub Form_Load()
    ...
    maryTemp = Split("0-不可能;1-必须;2-选择进行", ";")
    For mintCount = LBound(maryTemp) To UBound(maryTemp)
        Me.cbo病检.AddItem maryTemp(mintCount)
    Next
    Me.cbo病检.ListIndex = 0
    
    maryTemp = Split("0-不可能;1-必须;2-选择发放", ";")
    For mintCount = LBound(maryTemp) To UBound(maryTemp)
        Me.cbo胶片.AddItem maryTemp(mintCount)
    Next
...
Private Sub LoadClass()
    ...
    Set mrsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "查询诊疗分类项目")
    With mrsTemp
        If Not tvwClass.SelectedItem Is Nothing Then strCurrKey = tvwClass.SelectedItem.Key
        Me.tvwClass.Nodes.Clear
        Do While Not .EOF
            If IsNull(!上级ID) Then
                Set mobjNode = Me.tvwClass.Nodes.Add(, , "_" & !ID, "[" & !编码 & "]" & !名称, "close")
            Else
                Set mobjNode = Me.tvwClass.Nodes.Add("_" & !上级ID, tvwChild, "_" & !ID, "[" & !编码 & "]" & !名称, "close")
            End If
            mobjNode.Sorted = True
            mobjNode.ExpandedImage = "expend"
...
Private Sub tvwClass_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    err = 0: On Error GoTo errHand
    Set mrsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "查询诊疗项目目录", CLng(Mid(Node.Key, 2)))
    
    With mrsTemp
        Me.lvwItem.ListItems.Clear
        Do While Not .EOF
            Set mobjItem = Me.lvwItem.ListItems.Add(, "_" & !ID, !编码)
            mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_名称").Index - 1) = !名称
            mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_部位").Index - 1) = IIf(IsNull(!标本部位), "", !标本部位)
            mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_计算单位").Index - 1) = IIf(IsNull(!计算单位), "", !计算单位)
            .MoveNext
        Loop
    End With
    Exit Sub
