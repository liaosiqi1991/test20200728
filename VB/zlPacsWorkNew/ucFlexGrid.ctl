...
Public Function FormatValue(ByVal lngCol As Long, ByVal strValue As String) As String
    ...
    If Len(strValue) < mobjTmpDictionary(TColPro.cpAlignLen) Then
        strValue = zlStr.Lpad(strValue, mobjTmpDictionary(TColPro.cpAlignLen), mobjTmpDictionary(TColPro.cpAlignChar))
    End If
...
Public Sub SyncData(ByVal lngRow As Long, ByVal strColName As String, _
    ByVal strData As Variant, Optional ByVal blnUpdateAdo As Boolean = False)
'设置数据
    Dim blnFind As Boolean
    ...
    '更新界面数据显示
    Call GetFieldDisplayText(strColName, nvl(strData, ""), blnFind, chkState, strText, lngImgIndex)
...
On Error Resume Next
    Dim blnCustom As Boolean
        ...
        '101377 如果点击了检查过程，不进行排序
    If GetColName(Col) = "检查过程" Then Order = 0
        ...
        For i = 1 To mDataGrid.Rows - 1
            If mDataGrid.TextMatrix(i, lngCol) <> "" And i <> lngRow And Not mDataGrid.RowHidden(i) Then
                If mDataGrid.TextMatrix(i, lngCol) Like strValue & "*" Then
                    num = Val(zlStr.SubB(mDataGrid.TextMatrix(i, lngCol), _
    ...
    If mStrDefaultColNames <> "" And mStrDefaultColNames <> strCols Then
        ...
        For i = 0 To UBound(Cols)
            If Cols(i) <> "" Then
                ...
                If InStr("|" & mStrDefaultColNames & "|", "|" & strColName & ",") <= 0 And InStr("|" & mStrDefaultColNames & "|", "|" & strColName & "|") <= 0 Then
                    '删除不需加载的数据列
