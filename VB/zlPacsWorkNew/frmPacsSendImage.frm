...
Private Function SendFilesToDir(lngSeqUID As String, DicomDirPath As DicomDataSet, DestinationBoot As String, DestinationDir As String, Optional blnMoved As Boolean = False) As Boolean
    ...
    strCachePath = FormatFilePath(GetAppRootPath & "\Apply\TmpImage\")
    ...
    Do While Not rsTmp.EOF
        ...
        struFtpTag = FtpTagInstance(nvl(rsTmp("Host1")), nvl(rsTmp("User1")), nvl(rsTmp("Pwd1")), _
                                    objFile.GetParentFolderName(nvl(rsTmp("Root1")) & rsTmp("URL")))
                                    
        If Trim(struFtpTag.Ip) = "" Then
            struFtpTag = FtpTagInstance(nvl(rsTmp("Host2")), nvl(rsTmp("User2")), nvl(rsTmp("Pwd2")), _
                                        objFile.GetParentFolderName(nvl(rsTmp("Root2")) & rsTmp("URL")))
        End If
        
  
        lngResult = FtpDownload(struFtpTag, objFile.GetFileName(rsTmp("URL")), strTmpFile, False)
        ...
        If err.Number <> 0 Then
            WriteCommLog "SendFilesToDir", "图像发送", "复制[" & strCachePath & nvl(rsTmp("URL")) & "]到[" & DestinationBoot & DestinationDir & "]" & vbCrLf & _
