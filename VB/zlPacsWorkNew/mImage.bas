...
Public Function GetLineFtpInfo(ByVal strLineDeviceNo As String, ByRef dcmInfo As TDicomBaseInfo, ByRef strErr As String) As TFtpDeviceInf
    ...
    Else
        ...
        If blnIsGetNewDevice Then
            If Val(strLineDeviceNo) <= 0 Then
                strErr = "未找到图像存储设备,请确认在影像流程管理中是否对该科室配置了图像采集存储设备。"
                Exit Function
            End If
...
Public Function ResetStorageDevice(ByVal lngAdviceId As Long, ByRef objImgInf As clsBgImgInfo) As String
    Dim strSQL As String
    Dim rsData As ADODB.Recordset
...
Public Function GetDicomBaseInfo(ByVal lngAdviceId As Long) As TDicomBaseInfo
'获取Dicom基本信息
    Dim strSQL As String
    Dim rsData As ADODB.Recordset
