...
Public mcnOracle As New ADODB.Connection
...
Private Sub cmdSwitch_Click()
    ...
    '连接数据库
    If Not InitOraConn(strServerName, UCase(Trim(txtUserID.Text)), Trim(txtPassWord.Text), UCase(Trim(txtUserID.Text)) <> "SYS" And UCase(Trim(txtUserID.Text)) <> "SYSTEM") Then
    ...
    Set rsTemp = mcnOracle.Execute(strSQL)
    
    If rsTemp.EOF Then
        MsgBoxD Me, "当前用户未设置对应的人员信息,请与系统管理员联系,先到用户授权管理中设置！", vbInformation, gstrSysName
        Exit Sub
    Else
    ...
    strPrivs = GetPrivFuncByUser(100, mlngModule, rsTemp!用户名)
    If strPrivs = "" And UCase(strUserIDNew) <> "ZLHIS" Then
        MsgBoxD Me, "新用户不具备使用“影像工作站”" & mlngModule & "模块的权限！切换用户操作失败", vbInformation, gstrSysName
        Exit Sub
    End If
...
On Error GoTo ErrorHand
    If Not objRegister Is Nothing Then
        Set mcnOracle = objRegister.GetConnection(strServerName, strUserName, strUserPwd, blnTran, , strError)
    End If
