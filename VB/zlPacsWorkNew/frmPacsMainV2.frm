...
On Error GoTo errhandle
    ...
    Select Case lngEventNo
        ...
        Case WM_IMG_OPENVIEW
            ...
            If mobjCurStudyInfo.lngAdviceId = curData1 Then
                If mobjCurStudyInfo.strStudyUID <> "" Then
                    Call OpenViewer(1, mobjPacsCore, mobjCurStudyInfo.lngAdviceId, False, Me, "", mobjCurStudyInfo.blnMoved, mSysPar.blnLocalizerBackward)
                Else
                    Call OpenLatestImage(Me, mobjPacsCore, mobjCurStudyInfo, mSysPar.lngAutoImageDays, mSysPar.blnLocalizerBackward)
                End If
            Else
                Call GetSendNo(curData1, strStudyUID)
                
                If Len(strStudyUID) <= 0 Then
                    Call OpenLatestImage(Me, mobjPacsCore, GetBaseInfo(curData1), mSysPar.lngAutoImageDays, mSysPar.blnLocalizerBackward)
                Else
                    Call OpenViewer(1, mobjPacsCore, curData1, False, Me, , , mSysPar.blnLocalizerBackward)
                End If
            End If
        ...
        Case WM_IMG_CONTRASTVIEW
            ...
            Call OpenViewer(1, mobjPacsCore, curData1, True, Me, , , mSysPar.blnLocalizerBackward)
...
Private Function GetLastSignInfo(ByVal lngAdviceId As Long, Optional ByRef strCreateUser As String) As TReportSignInfo
    ...
    Call GetReportSignInfo(Val(nvl(rsData!ID)), reportSignInfo)
...
'On Error GoTo errHandle:
    ...
    Select Case lngEventNo
        Case BM_REPORT_EVENT_PRINT  '报告打印事件*****************************************************************************
            ...
            If Val(curData3) = -1 Then '-1表示病历编辑器
                If mSysPar.blnIgnoreResult = False Then Call ReportResultHint(curData1): blnSyncRow = True
                
                strSQL = "ZL_影像报告打印_Update(" & curData1 & ")"
                zlDatabase.ExecuteProcedure strSQL, "更新打印标记"
    
                If mSysPar.blnPrintCommit = True Then   '打印后直接完成
                    Call Menu_Manage_检查最终完成(Val(curData1), False): blnSyncRow = True
                End If
            ...
            Else
                ...
                Else
                    ...
                    If mSysPar.blnPrintCommit = True Then   '打印后直接完成
                        Call Menu_Manage_检查最终完成(Val(curData1), False)
                    End If
        ...
        Case BM_REPORT_EVENT_REJECT '报告驳回通知*****************************************************************************
            If Val(curData3) = -1 Or Val(strTag) = 1 Then
                Call Menu_Manage_SendAudit(curData1, "")    '该方法中会刷新列表行
...
'                End If
        ...
        Case BM_REPORT_EVENT_REFWCHR    '刷新词句
...
'判断视频是否嵌入的弹出式报告窗口
    ...
    lngVideoRootHwnd = GetAncestor(mobjWork_ImageCap.VideoHwnd, GA_ROOT)
...
Private Sub RefreshReportThumbnail()
    Dim objForm As Object
    
    For Each objForm In Forms
        If TypeOf objForm Is frm3DSetup Then
        End If
    Next
...
Private Function TraversalAdvice(ByVal lngAdviceId As Long, ByVal blnIsNext As Boolean, _
...
On Error GoTo errhandle
    ...
    strDllName = "zl9CardSquare.clsCardSquare"
...
On Error GoTo errhandle
    ...
    If mobjCurStudyInfo.blnMoved Then
        HintMsg "该病人的本次住院数据已经转出到后备数据库，不允许操作。", "Menu_Manage_取消检查完成", vbInformation
        Exit Sub
    End If
        ...
        With frmRISRequest
            ...
            If .mlngResultState <> 0 Then '成功返回
                ...
                If .mlngResultState = 2 Then
                    ...
                    Call AutoPrintApplication(.mlngAdviceId, .mlngSendNo, .mlngClinicID, .mintSourceType, .RequestInfo)
                End If
...
Private Sub AutoPrintApplication(lngAdviceId As Long, lngSendNo As Long, lngClinicId As Long, intSourceType As Long, RequestInfo As TRequestInfo)
'功能:根据能数据自动打印申请单
...
On Error GoTo errHand
    ...
    '是否启用自动避免重复打印
    If mSysPar.blnAutoPrintCheck Then
        If IsDate(RequestInfo.dtTime) And RequestInfo.lngPatiID > 0 And RequestInfo.strType <> "" Then
            strSQL = "SELECT A.医嘱ID FROM 病人医嘱发送 A,影像检查记录 B,病人信息 C" & vbNewLine & _
                        "Where A.医嘱ID=B.医嘱id And B.影像类别=[1] And A.报到时间>[2] And B.姓名=C.姓名 And C.病人ID=[3]"
            
            dtStart = DateAdd("n", -30, RequestInfo.dtTime)
            Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "获取打印信息", RequestInfo.strType, dtStart, RequestInfo.lngPatiID)
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "提取单据编号", lngClinicId, CLng(Decode(intSourceType, 3, 1, intSourceType)))  '诊疗单据项目使用场合需要区分住院，体检，门诊，外来默认为门诊
    
    strBillNo = "ZLCISBILL" & Format(rsTemp!编号, "00000") & "-1"

    If rsTemp.EOF Then Exit Sub
    If Not mSysPar.blnChoosePrintFormat Then
    '原来的方式
    ...
    '新的方式
        ...
        If strResult = "0" Then
            HintMsg "没有格式不能打印。", "AutoPrintApplication", vbInformation
        Else
    ...
    Exit Sub
errHand:
    If HintError(err, "AutoPrintApplication") = 1 Then Resume
End Sub
...
On Error GoTo errhandle
    ...
    If mlngCur科室ID <> Control.DescriptionText Or (Control.DescriptionText <> 0 And mblnAllDepts = True) Then
        ...
        Else
            ...
            If Not mobjWork_ImageCap Is Nothing Then
                Call mobjWork_ImageCap.zlInitModule(gcnOracle, mobjCapLinker, glngSys, mlngModule, mstrPrivs, mlngCur科室ID, Me.hwnd, gblnUseDebugLog)
            End If
            
            If Not mobjWork_Report Is Nothing Then
                Call mobjWork_Report.zlInitModule(Me, mlngModule, mstrPrivs, mlngCur科室ID, mobjCapLinker, ucPacsHelper1)
                '如果报告编辑器类型不同，则需要初始化
                If TabWindow.Selected.Caption = C_TAB_NAME_检查报告 Then
                    Call SelectModule(C_TAB_NAME_检查报告, strModuleTag)
...
'    Call Form_Resize
    ...
    If mstrSelTabName = C_TAB_NAME_影像采集 Then
        ...
        If GetAncestor(mobjWork_ImageCap.VideoHwnd, 1) = lngHwnd Then
            mobjCapLinker.ReportAdviceId = 0
            Exit Sub
        End If
...
'    End If
    ...
    If picDataSearchContainer.Width > lngBaseWidthDataSearchContainer Then
        ...
        Call picDataSearch.Move(lngMove, 0, lngWidth, picDataSearchContainer.Height)
        Call cmdDo.Move(lngMove + lngWidth + lngBaseWidth + intTMP)
    Else
        Call picDataSearch.Move(0, 0, lngWidth, picDataSearchContainer.Height)
        Call cmdDo.Move(lngWidth + lngBaseWidth + intTMP)
    End If
...
On Error GoTo errH
    ...
    Select Case imgFun(Index).ToolTipText
        ...
        Case C_FUNC_STR_观片
            ...
            Call OpenViewer(1, mobjPacsCore, mobjCurStudyInfo.lngAdviceId, True, Me, "", mobjCurStudyInfo.blnMoved, mSysPar.blnLocalizerBackward)
            ...
            Else
                ...
                Select Case strTmp
                    ...
                    Case C_WORKMODULE_NAME_病历报告 '电子病历编辑器
                        ...
                        If mobjRichReportWrap.InitEprEditor(Nothing, Me, mlngModule, GetCurDeptId) = False Then Exit Sub
            ...
            If Control.Parameter <> "" Then '执行发布到当前模块的报表
                ...
                If mobjCurStudyInfo.lngAdviceId <> 0 Then
                    Call ReportOpen(gcnOracle, Split(Control.Parameter, ",")(0), Split(Control.Parameter, ",")(1), Me, _
                        "NO=" & mobjCurStudyInfo.strNO, "性质=" & mobjCurStudyInfo.lngRecordKind, "医嘱id=" & mobjCurStudyInfo.lngAdviceId, 1)
                Else
...
'    mSysPar.lngCriticalValues = Val(GetDeptPara(lngDeptId, "危急情况判断", 0))           '危急情况判断
    ...
    mSysPar.blnLocalizerBackward = GetDeptPara(lngDeptId, "定位片后置", 0) = "1" '       '定位片后置
...
'Begin----------------------检查菜单--------------------------------------默认可见
    ...
    With cbrMenuBar.CommandBar
        ...
        Set cbrControl = CreateMenu(.Controls, xtpControlPopup, conMenu_Manage_Request, "申请单", "申请单", 0, False)
            ...
            Set cbrPopControl = CreateMenu(cbrControl.CommandBar.Controls, xtpControlButton, conMenu_Manage_CheckList, "查看电子申请", "查看电子申请单", 3564, False)
        ...
        If mlngModule <> G_LNG_PACSSTATION_MODULE Then
            Set cbrControl = CreateMenu(.Controls, xtpControlButton, conMenu_Tool_Analyse, "高级处理", "", 0, True): cbrControl.ToolTipText = "高级图像处理"
        Else
            Set cbrControl = CreateMenu(.Controls, xtpControlButton, conMenu_Img_OpenView, "观片", "", 8111, True): cbrControl.ToolTipText = "打开影像图像"
            Set cbrControl = CreateMenu(.Controls, xtpControlButton, conMenu_img_ContrastView, "对比", "", 8112, False): cbrControl.ToolTipText = "对比影像图像"
        End If
        Set cbrControl = CreateMenu(.Controls, xtpControlButton, conMenu_Check_ViewLink, "查看关联检查", "", 102, False): cbrControl.ToolTipText = "查看关联检查"
...
'        Set cbrControl = CreateMenu(.Controls, xtpControlButton, conMenu_Manage_LookRelatetion, "查看关联检查", "", , False)
        ...
        Set cbrControl = CreateMenu(.Controls, xtpControlButton, conMenu_Manage_ReportExecutor, "报告执行", "指定当前报告的记录人", 3967, True)
        ...
        If Not cbrControl Is Nothing Then
            ...
            Set cbrPopControl = CreateMenu(cbrControl.CommandBar.Controls, xtpControlButton, conMenu_Manage_Undone, "取消完成", "", 219, False)
    ...
    If mlngModule <> G_LNG_PACSSTATION_MODULE Then
        Set cbrControl = CreateMenu(cbrToolBar.Controls, xtpControlButton, conMenu_Tool_Analyse, "高级", "", 0, True): cbrControl.ToolTipText = "高级图像处理"
    Else
        Set cbrControl = CreateMenu(cbrToolBar.Controls, xtpControlButton, conMenu_Img_OpenView, "观片", "", 8111, True): cbrControl.ToolTipText = "打开影像图像"
        Set cbrControl = CreateMenu(cbrToolBar.Controls, xtpControlButton, conMenu_img_ContrastView, "对比", "", 8112, False): cbrControl.ToolTipText = "对比影像图像"
    End If
'    Set cbrControl = CreateMenu(cbrToolBar.Controls, xtpControlButton, conMenu_Check_ViewLink, "查看关联", "", 102, False): cbrControl.ToolTipText = "查看关联检查"
    ...
    With cbrControl.CommandBar
        ...
        Call CreateMenu(.Controls, xtpControlButton, conMenu_Manage_CheckList, "查看电子申请", "查看电子申请单", 3564, False)
    End With
...
'    Set cbrControl = CreateMenu(cbrToolBar.Controls, xtpControlButton, conMenu_Manage_SwitchUser, "切换", "切换用户", 3012, False, conMenu_Tool_Analyse)
    ...
    Set cbrControl = CreateMenu(cbrToolBar.Controls, xtpControlButton, conMenu_Manage_ReportExecutor, "报告执行", "指定当前报告的记录人", 3967, False)
...
'初始化ActivexExe视频采集模块对象
    ...
    If mobjWork_ImageCap Is Nothing Then
        Set mobjWork_ImageCap = CreateObject("zl9PacsImageCap.clsPacsCaptureV2") ' New zl9PacsCapture.clsPacsCapture
...
'            Call .RegEventObj(Me)
            ...
            Call .zlInitModule(gcnOracle, mobjCapLinker, glngSys, mlngModule, mstrPrivs, mlngCur科室ID, Me.hwnd, gblnUseDebugLog)
        End With
    End If
End Sub
...
Private Function VerifyModuleObj(ByVal strTabName As String, _
    ...
    Optional ByVal blnAutoCreate As Boolean = True) As Long
'验证模块对象
    Dim lngBound As Long
...
Private Function ReloadWorkModule(ByVal strTabName As String, _
    ...
    Optional ByRef objSelModule As Object) As Long
'重载工作模块
    ...
    lngSelHwnd = VerifyModuleObj(strTabName, objSelModule, strModuleTag)
...
On Error GoTo errhandle
    ...
    Call SelectModule(Item.Caption, strModuleTag)
    
    '保存实际使用的模块名称如住院医嘱，门诊医嘱，住院病历，门诊病历，病历报告等
    Item.tag = strModuleTag
    
    If mstrSelTabName = C_TAB_NAME_排队叫号 Then
        Call RefreshPacsQueueData(True)
    End If
...
Private Sub SelectModule(ByVal strTabName As String, ByRef strModuleTag As String)
    Dim objSel As Object
    
    mlngSelHwnd = ReloadWorkModule(strTabName, strModuleTag, objSel)
...
On Error GoTo errhandle
    ...
    Else
        ...
        If mblnAllDepts Then
            blNeedColStatistics = mobjPacsQueryWrap.DoBackgroundRefresh(mstrCanUse科室IDs)
        Else
            blNeedColStatistics = mobjPacsQueryWrap.DoBackgroundRefresh(mlngCur科室ID)
        End If
        
        '触发自动刷新后注意重新进行列统计
        If blNeedColStatistics Then Call mobjPacsQueryWrap.ExeColStatistics
...
Private Sub Menu_Manage_SendAudit(ByVal lngAdviceId As Long, ByVal strName As String)
    Dim strSQL As String
...
On Error GoTo errhandle
    ...
    If mblnIsHistoryMode Then mblnIsForceRefresh = True
    
    Call SetHistoryViewState(False)
    
    If Not mobjWork_Pathol Is Nothing Then
        Call mobjWork_Pathol.zlRefresh(mobjCurStudyInfo, strWorkModuleTag, mblnIsForceRefresh)
    End If
        
    Select Case strTabName
        Case C_TAB_NAME_影像图像
            Call objSelModule.zlRefreshFace(mobjCurStudyInfo, mblnIsForceRefresh)
        ...
        Case C_TAB_NAME_标本核收, C_TAB_NAME_病理取材, C_TAB_NAME_病理制片, C_TAB_NAME_病理特检, C_TAB_NAME_过程报告
            ...
            Call objSelModule.zlRefreshFace(mblnIsForceRefresh)
            
        Case C_TAB_NAME_检查报告
            Call mobjWork_Report.zlRefreshFace(mobjCurStudyInfo, strWorkModuleTag, mblnIsForceRefresh)
            
        Case C_TAB_NAME_费用记录, C_TAB_NAME_医嘱记录, C_TAB_NAME_病历记录, C_TAB_NAME_电子病历
            Call mobjWork_His.zlModule.zlRefresh(mobjCurStudyInfo, strWorkModuleTag, mblnIsForceRefresh)
    ...
    If Not (dkpMain.Panes(2).hidden = False And ucPacsHelper1.Visible = False) Then
        ...
        Call ucPacsHelper1.zlRefresh(mobjCurStudyInfo, 0, mblnIsForceRefresh)
    End If
...
'根据科室ID更新一个全局参数
    ...
    Else
        gblUseImgSignValid = GetDeptPara(lngID, "图像签名验证") = "1"
    End If
errH:
End Sub
