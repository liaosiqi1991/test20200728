...
Private Sub Menu_PatholMeal_Save()
    ...
    With ufgMeal
        ...
        .DataGrid.Col = 5
        .DataGrid.Row = .SelectionRow
        .DataGrid.SetFocus
    
        If LenB(StrConv(.Text(.SelectionRow, gstrAntibodyMeal_套餐名称), vbFromUnicode)) > 64 Then
            MsgBoxD Me, "套餐名称过长，应小于64个英文或32个汉字", vbExclamation, Me.Caption
            Exit Sub
        End If
        
        If LenB(StrConv(.Text(.SelectionRow, gstrAntibodyMeal_套餐类别), vbFromUnicode)) > 64 Then
            MsgBoxD Me, "套餐类别过长，应小于64个英文或32个汉字", vbExclamation, Me.Caption
            Exit Sub
        End If
        
        If LenB(StrConv(.Text(.SelectionRow, gstrAntibodyMeal_套餐说明), vbFromUnicode)) > 1024 Then
            MsgBoxD Me, "套餐说明过长，应小于1024个英文或512个汉字", vbExclamation, Me.Caption
            Exit Sub
        End If
        
        '检查套餐名称是否为空
        If .Text(.SelectionRow, gstrAntibodyMeal_套餐名称) = "" Then
            MsgBoxD Me, "未能通过验证，原因是套餐名称不能为空！", vbExclamation, Me.Caption
            .LocateRow .SelectionRow
            .DataGrid.EditCell
            Exit Sub
        Else
            '检查套餐名称是否重复
