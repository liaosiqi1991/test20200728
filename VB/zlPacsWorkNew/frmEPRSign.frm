...
Private signInfo As TReportSignInfo

Private mlngRegID As Long
Private mintPatiType As Integer
...
Private mstrImgSingInfo As String '图像base64信息
...
Public Function ShowMe(ByRef fParent As Object, ByVal lngPassType As Long, ByVal lngReportID As Long, ByVal lngPatientID As Long, ByVal lngPageID As Long, _
     ByVal strPrivs As String, ByVal lngMaxSignLevel As Long, int开始版 As Integer, ByVal strImgSingInfo As String, ByVal lngAdviceId As Long, Optional ByVal intPatiType As Integer, Optional ByVal lngRegID As Long) As cEPRSign
    Dim curDate As Date
    ...
    mblnOk = False
    mstrImgSingInfo = strImgSingInfo
    mlngAdviceID = lngAdviceId
    mlngRegID = lngRegID
    mintPatiType = intPatiType
    ...
    '根据权限，确定是否显示“审核签名”按钮
    cmdOk(2).Visible = CheckPopedom(mstrPrivs, "PACS报告修订") Or CheckPopedom(mstrPrivs, "PACS报告审核")
...
Public Function ShowSign(ByRef fParent As Object, ByVal lngPassType As Long, _
    ByVal lngReportID As Long, ByVal strPrivs As String, _
    ByVal lngMaxSignLevel As Long, ByVal strFirstSign As String, int开始版 As Integer, _
     ByVal strImgSingInfo As String, ByVal lngAdviceId As Long) As TReportSignInfo
     
    Dim curDate As Date
    Dim intFormat As Integer
    Dim strSQL As String
    Dim rsTemp As ADODB.Recordset
    Dim blnIsAuditing As Boolean
...
On Error GoTo errhandle
    ...
    UserSignLevel = GetUserSignLevel(UserInfo.ID, , mlngPatientId, mlngPageID)    '获取用户签名级别
    blnIsAuditing = CheckPopedom(mstrPrivs, "PACS报告审核") Or CheckPopedom(mstrPrivs, "PACS报告终审")
    '根据当前报告的签名状态，确定是否显示“诊断签名”按钮，当已经有过一次审核签名后，不再显示诊断签名按钮
    If lngMaxSignLevel > 1 Then
        '没有审核权限但有修订权限的处理
        If Not blnIsAuditing And CheckPopedom(mstrPrivs, "PACS报告修订") And UserSignLevel > lngMaxSignLevel Then
            cmdOk(1).Caption = "修订签名"
            cmdOk(1).Visible = True
        Else
            cmdOk(1).Visible = False
        End If
    Else
        '当前用户和首次诊断签名用户不同时，则不能进行诊断签名
        If strFirstSign <> "" And UserInfo.姓名 <> strFirstSign Then
            '已被他人修订,当前签名级别高于之前的级别且具备修订权限，则可进行修订签名
            If Not blnIsAuditing And CheckPopedom(mstrPrivs, "PACS报告修订") And UserSignLevel > lngMaxSignLevel Then
                cmdOk(1).Caption = "修订签名"
                cmdOk(1).Visible = True
            Else
    ...
    '根据权限，确定是否显示“审核签名”按钮(有报告和修订权限时，均可审核签名)
    cmdOk(2).Visible = blnIsAuditing 'CheckPopedom(mstrPrivs, "PACS报告修订") Or
...
Private Function Validation() As Boolean
    ...
    '使用指定用户签名
        If chkEsign.value = vbUnchecked Then
            ...
            If gobjRegister.LoginValidate(strServerName, txtName, txtPass, strErr) = False Then
    ...
    If chkEsign.value = vbChecked Then
        ...
        Else
            '获取签名的源文
            intRule = GetSignSourceString(1, mlngReportID, mint开始版, False, Sign, strSource, mstrImgSingInfo, mlngAdviceID)
            If intRule = 0 Then
                '源文提取失败，退出签名
            ...
            strSign = gobjESign.Signature(strSource, strDBUser, lngCertID, str时间戳, objPic, str时间戳Base64,, mlngPatientId, IIf(mintPatiType = 2, mlngPageID, 0), IIf(mintPatiType = 1, mlngRegID, 0))
            If strSign = "" Then
                MsgBoxD Me, "验证失败！请重新输入验证信息！", vbInformation + vbOKOnly, "书写签名"
                GoTo LL
            End If
    ...
    signInfo.ID = Sign.ID
    signInfo.Key = Sign.Key
    signInfo.对象序号 = Sign.对象序号
    signInfo.对象属性 = Sign.对象属性
    signInfo.父ID = Sign.父ID
    signInfo.开始版 = Sign.开始版
    signInfo.签名级别 = Sign.签名级别
    signInfo.签名人ID = Sign.签名人ID
    signInfo.签名图片 = Sign.签名图片
    signInfo.签名信息 = Sign.签名信息
    signInfo.前置文字 = Sign.前置文字
    signInfo.时间戳 = Sign.时间戳
    signInfo.时间戳信息 = Sign.时间戳信息
    signInfo.证书ID = Sign.证书ID
    signInfo.姓名 = Sign.姓名
    signInfo.签名方式 = Sign.签名方式
    signInfo.签名规则 = Sign.签名规则
    signInfo.显示手签 = Sign.显示手签
    signInfo.签名时间 = Sign.签名时间
    signInfo.显示时间 = Sign.显示时间
