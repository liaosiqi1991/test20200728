...
Private Sub lbl病理诊断_DblClick()
    ...
    If Not mfrmParentReport Is Nothing Then
       Call mfrmParentReport.WordItemClick(ReportViewType_病理诊断, ReportViewType_病理诊断, txtPathologyDiag.Text)
    End If
err:
...
Private Sub lbl活检部位_DblClick()
    On Error GoTo err
    If Not mfrmParentReport Is Nothing Then
        Call mfrmParentReport.WordItemClick(ReportViewType_活检部位, ReportViewType_活检部位, txt活检部位.Text)
    End If
err:
End Sub
