...
Public Sub ChangeSepState(ByVal blnState As Boolean, ByVal blnIsForceRefresh As Boolean)
    ...
    If mblnIsSpeState Then
        If dkpMain.Panes(5).ID = mlngAdviceId And blnIsForceRefresh = False Then Exit Sub
...
'        End If
    ...
    '读取报告签名等相关信息****************************************
    If mlngReportID <> 0 Then
        Call ReadResultTag(mlngAdviceId, 0) '支持多报告情况下，才需要传递报告ID
...
Private Sub ReadResultTag(ByVal lngAdviceId As Long, Optional ByVal lngReportID As Long = 0)
'读取结果标记
'结果标记包含阴阳性，危急状态等信息
    Dim strSQL As String
    Dim rsData As ADODB.Recordset
...
On Error GoTo errH
    ...
    While rsTemp.EOF = False
        If GetReportSignInfo(Val(rsTemp!ID), reportSignInfo) Then
...
'读取标记图像
    ...
    strFileName = "标记图_" & reportImgTag.lngFileId & "_" & reportImgTag.strKey & ".JPG"
...
Private Function DownLoadFtpFile(ByVal lngAdviceId As Long, ByVal strFtpFile As String, ByVal strLocalFile As String) As ftpResult
'下载ftp文件
    DownLoadFtpFile = frNormal
    If Len(mftpConTag.Ip) <= 0 Or Val(mftpConTag.tag) <> lngAdviceId Then
        mftpConTag = GetReportDevice(lngAdviceId)
...
Private Function UpLoadFtpFile(ByVal lngAdviceId As Long, ByVal strFtpFile As String, ByVal strLocalFile As String) As ftpResult
'上传ftp文件
...
Private Function GetReportDevice(ByVal lngAdviceId As Long) As TFtpConTag
    Dim strSQL As String
    Dim rsData As ADODB.Recordset
...
'读取报告图
    ...
    If Len(strFileName) > 0 Then
        ...
        If FileExists(strFile) = False Then
            ...
            ReadReportImage = DownLoadFtpFile(lngAdviceId, strFileName, strFile)
...
On Error GoTo errhandle
    ...
    If Not mobjSpePlugin Is Nothing Then
        If mobjSpePlugin.zlReportAction(lngReportID, 1) = False Then Exit Function   '删除报告
    End If
...
'写入报告图
    ...
    For i = 1 To iImgCount
        ...
        ftpResult = UpLoadFtpFile(lngImageAdiceId, reportImgTag.strImgFile, strLocalFile)
...
'    End If
    ...
    If Not mobjSpePlugin Is Nothing Then
        Call mobjSpePlugin.zlReportAction(mlngReportID, 0)   '保存报告
...
'-----------------------------------------------
    ...
    '提取源文
    strSource = GetSignSource(mlngReportID, lngSignVer)
...
Private Function GetSignSource(ByVal lngReportID As Long, ByVal int签名版本 As Integer) As String
'------------------------------------------------
    ...
    If intRule = 1 Then
        ...
        If mblnUseImgSign Then
            ...
            If lngImgAdviceId <= 0 Then
                strSignImgBase64 = GetSignedImgB64(strImgFiles, mlngAdviceId)
            Else
                strSignImgBase64 = GetSignedImgB64(Split(strImgFiles, "[ADV]")(0), lngImgAdviceId)
            End If
...
Private Function GetSignedImgB64(ByVal strSignImgFiles As String, ByVal lngImgAdviceId As Long) As String
'获取图像的base64
    Dim i As Long
    ...
    For i = 0 To UBound(aryFile)
        If Len(aryFile(i)) > 0 Then
            ...
            If FileExists(strLocalFile) = False Then
                If DownLoadFtpFile(lngImgAdviceId, aryFile(i), strLocalFile) <> frNormal Then
                    Exit Function
                End If
            End If
...
'签名回退
    ...
    If signInfo.Key > 0 Then
        ...
        '清除签名,并保存格式
        If SaveSignFormat(mlngReportID, signInfo, "", True) = False Then Exit Function
...
On Error GoTo errhandle
    ...
    '签名格式保存成功后，需要递增版本号
    If SaveSignFormat(mlngReportID, signInfo, strImgFiles) Then
...
Private Function SaveSignFormat(ByVal lngReportID As Long, signInfo As TReportSignInfo, _
...
Private Function GetSignMarkImgName(ByVal lngReportID As Long, ByVal intSignVer As Integer) As String
    GetSignMarkImgName = "标记图_" & lngReportID & "_" & intSignVer & ".JPG"
End Function
...
'保存签名图像
    ...
    If UpLoadFtpFile(lngImgAdviceId, strFileName, strFile) <> frNormal Then
...
errhandle:
    GetSignImgEncode = False
    MsgboxH GetRootHwnd, "图像Base64转换错误，不能进行签名。" & vbCrLf & err.Description, vbOKOnly, "提示"
End Function
...
'只读状态下的报告，肯定不能进行编辑
    ...
    If mblnTechReptSame Then
        ...
        If nvl(rsTemp!检查技师) <> "" And nvl(rsTemp!检查技师) <> UserInfo.姓名 Then
            labEditState.Caption = "无权对[" & nvl(rsTemp!检查技师) & "]的检查书写"
            labEditState.ForeColor = vbRed
...
Public Sub AddRepImage(objDcmImg As Object, Optional ByVal lngReleationImageAdviceId As Long = 0, Optional ByVal strFileName As String = "")
'添加报告图
...
'添加报告图文件
    ...
    Call AddRepImage(objCurDicom, lngImageAdviceId, strFileName)
End Sub
    ...
    If blnIsUseSpecialty = False Then
        If Len(strFreeText) > 0 Then
            If Not mrtbActive Is Nothing Then
                If mrtbActive.Enabled And mrtbActive.Locked = False And mrtbActive.Visible Then mrtbActive.SelText = strFreeText
            End If
        End If
...
Public Sub LocateEditBox()
    ...
    If objActive Is Nothing Then Set objActive = mrtbActive
...
errhandle:
    ...
    MsgboxH GetRootHwnd, "专科报告初始化失败：" & err.Description, vbOKOnly, "提示"
End Function
...
Private Sub dcmReportImg_MouseMove(Button As Integer, Shift As Integer, X As Long, Y As Long)
    ...
    picReportImgOper.Visible = blnIsReportImgArea
