...
Public Function GetFileFormatId(ByVal lngAdviceId As Long) As Long
'获取检查对应的诊疗单据格式ID
...
'刷新
    ...
    If mlngFileID = 0 Then mlngFileID = GetFileFormatId(mobjStudyInfo.lngAdviceId)
    
    Call RefreshHelperComponent(mobjStudyInfo.lngAdviceId, mobjStudyInfo.strStudyUID, mlngFileID)
...
Private Sub RefreshHelperComponent(ByVal lngAdviceId As Long, ByVal strSutdyUid As String, ByVal lngFileId As Long)
'刷新帮助组件
    If mobjSel Is Nothing Then Exit Sub
    Select Case mobjSel.hwnd
        Case ucImages.hwnd  '图像
            ...
            '判断检查是否已经有图像
            If Len(strSutdyUid) > 0 Then Call LoadExamImages(lngAdviceId)
            ...
            If ucImages.ImgCount <= 0 Then
                If mlngReleationImgDays > 0 Then
                    ...
                    If mlngReleationImgAdvice > 0 Then
                        Call LoadExamImages(mlngReleationImgAdvice)
                    End If
                Else
                    mstrReportImageUids = ""
...
Public Sub ClearReportImgState()
'清除报告图状态
...
'同步报告图状态
    Call ucImages.SyncDrawHint(strImgs, "报")
...
Private Sub LoadExamImages(ByVal lngAdviceId As Long)
'载入检查图像
    ...
    strResult = ResetStorageDevice(lngAdviceId, objImgInf)
    ...
    Set rsData = GetExamImgData(strStudyUID)
...
Private Function GetExamImgData(ByVal strStudyUID As String) As ADODB.Recordset
'获取检查图像数据
    Dim strSQL As String
    Dim rsData As ADODB.Recordset
...
On Error GoTo errhandle
    ...
    If mlngModuleNo = G_LNG_PACSSTATION_MODULE Then
        ...
        strSQL = "  Select 1 as 序列号, Replace(Trim(B.Column_Value),'.jpg','') as 图像UID, rownum as 图像号, 'REPIMG' as 图像描述, 0 as 报告图, 5 as 动态图, " & _
...
Private Sub ucImages_OnCmdEvent(ByVal strCmd As String)
    If strCmd = "REFRESH" Then Call LoadExamImages(mobjStudyInfo.lngAdviceId)
End Sub
...
Private Sub RefreshData()
    Select Case tabSelect.Selected.tag
        Case CON_TAB_TAG_图像
            If Len(mobjStudyInfo.strStudyUID) > 0 Then Call LoadExamImages(mobjStudyInfo.lngAdviceId)
            
            If ucImages.ImgCount <= 0 Then
                If mlngReleationImgAdvice <> 0 Then
                    LoadExamImages (mlngReleationImgAdvice)
                Else
                    mstrReportImageUids = ""
                    ucImages.ClearAll
                End If
    ...
    For i = UBound(arySelIndex) To 1 Step -1
        ...
        If Not objDcmImg Is Nothing Then
            If Len(objDcmImg.InstanceUID) > 0 Then
                ...
                Call ucCache.DeleteCacheImg(arySelIndex(i))
            End If
        End If
    Next
...
Private Sub ImageProcess()
    ...
    Call ShowImageProcess(arySelIndex(1), ptProcess)
End Sub
...
Private Function SaveDicomImageToStudy(objDcmImg As DicomImage, _
    ...
    dcmInfo = GetDicomBaseInfo(mobjStudyInfo.lngAdviceId)
    
    Call WriteDicomPara(objDcmImg, dcmInfo)
    
    lineFtpInfo = GetLineFtpInfo(strLineDeviceNo, dcmInfo, strError)
...
On Error GoTo errH
    ...
    For i = UBound(arySelIndex) To 1 Step -1
        ...
        If InStr(mstrReportImageUids, ";" & objImgInfo.Key & ";") <= 0 Then
            ...
            Case CON_TAB_TAG_图像 '**********************************************************************************
                ...
                Else
                    If FileExists(strImgFailedFile) Then
                        Set cbrControl = .Add(xtpControlButton, conMenu_Helper_ImageProcess, "传输日志(&L)")
...
On Error GoTo errhandle
    ...
    If blnShowImg Then        '显示图像
        ...
        If objImgInfo.LoadState = lsError Or objImgInfo.LoadState = lsRedo Or objImgInfo.LoadState = lsSent Then Exit Sub
...
Public Sub ShowImageProcess(ByVal lngImgIndex As Long, ByVal lngType As TImgProcessType)
    Dim i As Long
    Dim objDcmImg As DicomImage
