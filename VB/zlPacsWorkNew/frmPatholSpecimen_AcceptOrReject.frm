...
On Error GoTo errH
    ...
    Dim strUnit As String     '病理送检信息 送检单位
    Dim strSentPerson As String '病理送检信息 送检人
    Dim strSentDepart As String '病理送检信息 送检科室
    
    If blnShowSentInfo Then
    
        strSQL = "select 送检单位,送检科室,送检人 from 病理送检信息 where 医嘱ID=[1] and 送检日期=to_date('1000/10/10 10:10:10','yyyy/mm/dd hh24:mi:ss')"
        Set rsData = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lngAdviceID)
        If rsData.RecordCount = 1 Then
            strUnit = Trim(nvl(rsData!送检单位))
            strSentPerson = Trim(nvl(rsData!送检人))
            strSentDepart = Trim(nvl(rsData!送检科室))
        End If
    Else
        strUnit = "本院"
    End If
...
On Error GoTo errHandle
    ...
    If int记录性质 = 2 Then
        ...
        If curMoney <> 0 Then
            '记账报警
            If Not FinishBillingWarn(Me, ";" & GetPrivFunc(100, 1257) & ";", lngPatientID, rsTemp!主页ID, Val(nvl(rsTemp!当前病区ID)), curMoney, str类别, str类别名) Then
                AutoRegister = False
                Exit Function
            End If
            ...
            '参数81--执行后自动审核
            If curMoney > 0 And lngSourceType = 1 Then
                If Not zlPatiIdentify(1294, Me, lngPatientID, curMoney) Then
                    AutoRegister = False
                    Exit Function
                End If
            End If
    ...
    arrSQL(UBound(arrSQL)) = "ZL_影像检查_BEGIN(Null,Null," & mlngCurAdviceId & "," & rsTemp!发送号 & ",'DG','" & _
        ...
        nvl(rsTemp!年龄, "") & "'," & zlStr.To_Date(nvl(rsTemp!出生日期, "")) & ",Null,Null,Null,Null,Null,Null,Null,Null,Null," & _
        mstrCurDepartmentId & ")"
    ...
    If mlngPatholStationMoneyExeModle = 0 Then
        ReDim Preserve arrSQL(UBound(arrSQL) + 1)
        arrSQL(UBound(arrSQL)) = "Zl_影像费用执行(" & mlngCurAdviceId & "," & rsTemp!发送号 & ",2,Null,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & mstrCurDepartmentId & ")"
    End If
...
On Error GoTo errHandle
    ...
    mlngPatholStationMoneyExeModle = Val(zlDatabase.GetPara("病理费用执行模式", glngSys, G_LNG_PATHOLSYS_NUM, 0))
