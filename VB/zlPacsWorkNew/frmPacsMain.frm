...
Object = "*\A..\..\..\..\ZLCommon10\U10.35.0\zlIDKind\zlIDKind.vbp"
...
Private mblnDBLink As Boolean              '是否DBLink连接
Private mblnRefreshWord As Boolean              '是否强制刷新词句界面

Property Get StartDate() As Date
    StartDate = SQLCondition.开始时间
    If StartDate = Empty Then StartDate = DateAdd("d", -3, Now)
End Property

Property Get EndDate() As Date
    EndDate = SQLCondition.结束时间
    If EndDate = Empty Then EndDate = Now
End Property
...
Public Sub ShowStation(ByVal lngModule As Long, Owner As Object)
    ...
    mblnDBLink = Sys.CheckDatamoveRemote
...
On Error GoTo errhandle
    ...
    With ufgStudyList
        ...
        If Not mobjWork_Report Is Nothing Then
            Call mobjWork_Report.zlUpdateAdviceInf(mListAdviceInf.lngAdviceId, mListAdviceInf.lngPatId, mListAdviceInf.lngSendNo, mListAdviceInf.intStep, mListAdviceInf.intMoved = 1, mListAdviceInf.lngBaby)
...
On Error GoTo errhandle
    ...
    Else
        frmSelectMuli.ShowImageReleation mlngModule, mListAdviceInf.lngAdviceId, mstrPrivs, mListAdviceInf.intMoved = 1, IIf(mlngModule = G_LNG_PACSSTATION_MODULE, False, True), mlngCur科室ID, 1
...
'混合模式下使用
    ...
    objPatholPrice.zlRefresh mlngCur科室ID, mListAdviceInf.lngAdviceId, mListAdviceInf.lngSendNo, mListAdviceInf.intMoved = 1
...
On Error GoTo errhandle
    ...
    If mSysPar.blnFinishCommit And CheckPopedom(mstrPrivs, "检查完成") Then  '无报告完成后无需再次确认完成,但需要有检查完成的权限
        ...
        If mrtReportType = 报告文档编辑器 Then
            gstrSQL = "Zl_影像检查_状态更新(" & mListAdviceInf.lngAdviceId & "," & mListAdviceInf.lngSendNo & ",'',6,1,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & lngID & "," & zlStr.To_Date(zlDatabase.Currentdate) & ")"
        ElseIf mrtReportType = PACS报告编辑器 Then
            gstrSQL = "ZL_影像检查_STATE(" & mListAdviceInf.lngAdviceId & "," & mListAdviceInf.lngSendNo & ",6,1,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & lngID & "," & zlStr.To_Date(zlDatabase.Currentdate) & ",1)"
        Else
            gstrSQL = "ZL_影像检查_STATE(" & mListAdviceInf.lngAdviceId & "," & mListAdviceInf.lngSendNo & ",6,1,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & lngID & "," & zlStr.To_Date(zlDatabase.Currentdate) & ",2)"
        End If
    Else
        If mrtReportType = 报告文档编辑器 Then
...
On Error GoTo errH
    ...
    If (lngIndex <= 0 And lngAdviceId > 0) Or blnFromDB = True Then
        ...
        If zlDatabase.TableDataMoved("病人医嘱记录", "ID", " = " & lngAdviceId) Then
            strSQLBak = strSQL
    ...
    '        strSQLBak = Replace(strSQLBak, "病人信息", "H病人信息")            '病人信息表并未参与转存
        ...
        If Not rsTemp.EOF Then
            With GetAdviceDetailInf
                ...
                .strMarkNum = IIf(.lngPatientFrom = 1, nvl(rsTemp!门诊号), IIf(.lngPatientFrom = 2, nvl(rsTemp!住院号), "0"))
    ...
    With GetAdviceDetailInf
        ...
        If mrtReportType = 报告文档编辑器 Then
            ...
            End If
        Else
            .blnCanPrint = IIf(mSysPar.blnCanPrint, IIf(ufgStudyList.Text(lngIndex, "紧急") = "  ", ufgStudyList.Text(lngIndex, "报告人") <> "", ufgStudyList.Text(lngIndex, "复核人") <> "" Or ufgStudyList.Text(lngIndex, "绿色通道") = "  "), True)
        End If
        ...
        .strMarkNum = ufgStudyList.Text(lngIndex, "标识号")
...
On Error GoTo errhandle
    ...
    If mrtReportType = 报告文档编辑器 Then
        strSQL = "Zl_影像检查_状态更新(" & curAdviceInf.lngAdviceId & "," & curAdviceInf.lngSendNo & ",'',6,NULL,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & lngID & "," & zlStr.To_Date(zlDatabase.Currentdate) & ")"
    ElseIf mrtReportType = PACS报告编辑器 Then
        strSQL = "ZL_影像检查_STATE(" & curAdviceInf.lngAdviceId & "," & curAdviceInf.lngSendNo & ",6,NULL,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & lngID & "," & zlStr.To_Date(zlDatabase.Currentdate) & ",1)"
    Else
        strSQL = "ZL_影像检查_STATE(" & curAdviceInf.lngAdviceId & "," & curAdviceInf.lngSendNo & ",6,NULL,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & lngID & "," & zlStr.To_Date(zlDatabase.Currentdate) & ",2)"
    End If
...
On Error GoTo ErrorHnad
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord("select Id, 方案名称, 是否默认 from 影像查询方案 where 使用状态=1 and 所属模块 is null and (所属科室=0 or 所属科室 is null or 所属科室=[1]) order by 所属科室 desc, 方案序号", "创建查询菜单", lngDeptId)
...
On Error GoTo errhandle
    ...
    If blnIsLoading = True Or ufgStudyList.IsLoading = True Then
        MsgBoxD Me, "数据加载中，请稍后重试...", vbInformation, gstrSysName
        Exit Sub
    End If
...
On Error GoTo errhandle
    ...
    Select Case lngEventType
        ...
        Case TVideoEventType.vetAddReportImg
            '加入报告图
            If Not mobjWork_Report Is Nothing Then
                Call mobjWork_Report.UpdateVideoCaptureState(lngEventType, lngAdviceId, strOther, dcmImage)
            End If
    End Select
Exit Sub
errhandle:
...
Private Sub mobjQueue_OnCalled(ByVal lngAdviceId As Long, ByVal strRoom As String, ByVal TCallWay As zlQueueOper.TCallWay)
    ...
    Dim rsTemp As ADODB.Recordset
    Dim rsData As ADODB.Recordset
    Dim blnRsDataHaveThisData As Boolean '记录集是否存在对应数据
    Dim blnListHaveThisData As Boolean '列表中是否显示对应数据
...
'---------------------------检查-----------------
        ...
        Case conMenu_Tool_Analyse
            Call OpenViewer(1, mobjPacsCore, mcurAdviceInf.lngAdviceId, True, Me, "", mListAdviceInf.intMoved = 1, mSysPar.blnLocalizerBackward)
    ...
    For i = 0 To UBound(ary)
        ...
        Do While InStr(strTmpVBS, "[[") > 0
            ...
            Select Case strParaName
                ...
                Case "[[门诊号]]", "[[住院号]]"
                    strParaVal = mcurAdviceInf.strMarkNum
...
'功能：取得当前过滤的SQL
    ...
    Dim blnReportType As Boolean    '是否为报告文档编辑器
    Dim strReportFilter As String   '报告文档编辑器报告相关过滤
    Dim strSubReportFilter As String
    Dim strSubHeadString As String
    Dim strSubTailString As String
    ...
    With SQLCondition
        ...
        ElseIf .姓名 <> "" Then     '姓名特殊处理，带*号表示模糊查询
            ...
            If mblnDBLink Then
                strFilterOrder = " And C.姓名 like '" & .姓名 & "'"
            Else
        ...
        '其他条件查询，使用时间索引
            ...
            blnReportType = mrtReportType = 报告文档编辑器
                
            If blnReportType Then
                strSubHeadString = "Select b.医嘱id" & vbNewLine & _
                                    "From (Select t.医嘱id, Extractvalue(Value(m), '/utext') Re, Rownum Rn" & vbNewLine & _
                                    "       From 病人医嘱发送 d, 影像报告记录 t, Table(Xmlsequence(Extract(t.报告内容, '/zlxml/document//section"
                strSubTailString = "/utext'))) m" & vbNewLine & _
                                "       Where d.医嘱id = t.医嘱id And d.发送时间 Between [10] And [11]) b" & vbNewLine
            ...
            If .结果阳性 <> -1 Then
                If blnReportType Then
                    If strFilterStudy <> "" Then strFilterStudy = strFilterStudy & " AND "
                    strFilterStudy = strFilterStudy & " Nvl(S.结果阳性, 0)=[30]"
                    blnState = True
                Else
                    strFilterStudy = strFilterOrder & " And Nvl(A.结果阳性, 0)=[30]"
                End If
            End If
            ...
            If .诊断医生 <> "" Then
                ...
                If blnReportType Then
                    strFilterStudy = strFilterStudy & " S.创建人=[14] "
                    blnState = True
                Else
            ...
            If .审核医生 <> "" Then
                ...
                If blnReportType Then
                    strFilterStudy = strFilterStudy & " S.最后审核人=[15] "
                    blnState = True
                Else
            ...
            If .待处理人 <> "" Then
                ...
                If blnReportType Then
                    strFilterStudy = strFilterStudy & " NVL(S.待处理人,H.待处理人)=[32] "
                    blnState = True
                Else
            ...
            If .检查所见 <> "" Then
                If Not blnReportType Then
                    strFilterTemp = " (b.内容文本 ='检查所见' And Instr(c.内容文本, [21]) > 0)"
                Else
                    strSubReportFilter = strSubReportFilter & IIf(Len(strSubReportFilter) = 0, "", " union all ") & strSubHeadString & "[@title=""检查所见""] " & strSubTailString & _
                                "Where Instr(b.Re, [21]) > 0"
                End If
            End If
            ...
            If .诊断意见 <> "" Then
                If Not blnReportType Then
                    ...
                    End If
                Else
                    strSubReportFilter = strSubReportFilter & IIf(Len(strSubReportFilter) = 0, "", " union all  ") & strSubHeadString & "[@title=""诊断意见""] " & strSubTailString & _
                                "Where Instr(b.Re, [22]) > 0"
                End If
            End If
            ...
            If .建议 <> "" Then
                If Not blnReportType Then
                    ...
                    End If
                Else
                    strSubReportFilter = strSubReportFilter & IIf(Len(strSubReportFilter) = 0, "", " union all  ") & strSubHeadString & "[@title=""诊断建议""] " & strSubTailString & _
                                "Where Instr(b.Re, [23]) > 0"
                End If
            End If
            ...
            If Len(strSubReportFilter) > 0 And blnReportType Then
                strReportFilter = strReportFilter & " and l.id in(" & strSubReportFilter & ")"
                blnState = True
            End If
        End If
        ...
        '是否选择了全部科室
        If mblnAllDepts = True Then
            strFilterOrder = strFilterOrder & " And Instr( [25],b.执行科室id ) >0"
            
            If lngType = 2 Then
                If strFilterStudy <> "" Then strFilterStudy = strFilterStudy & " AND "
                strFilterStudy = strFilterStudy & "  Instr([25],b.执行科室id) >0 "
            End If
        Else
            strFilterOrder = strFilterOrder & " And b.执行科室id+0=[24]"
            
            If lngType = 2 Then
                If strFilterStudy <> "" Then strFilterStudy = strFilterStudy & " AND "
                strFilterStudy = strFilterStudy & "  b.执行科室id+0=[24] "
            End If
        End If
        ...
        If .报告内容 <> "" Then
            ...
            Else
                strReportFilter = strReportFilter & " and l.id in(" & strSubHeadString & strSubTailString & _
                                "Where Instr(b.Re, [26]) > 0)"
                blnState = True
            End If
        End If
        ...
        If lngType = 1 Then
            ...
            strWithStudyTab = "tmpStudy as(select " & strWithStudyCols & vbNewLine & _
             ...
             If blnReportType And blnState Then
                strWithStudyTab = Replace(strWithStudyTab, "where h.医嘱ID=I.医嘱ID", "where h.医嘱ID=s.医嘱ID(+) and  h.医嘱ID=I.医嘱ID")
                strWithStudyTab = Replace(strWithStudyTab, "from 影像检查记录 H", "from 影像检查记录 H,影像报告记录 S")
             End If
        ...
        Else
            ...
            If blnReportType And blnState Then strStudyTabWhere = strStudyTabWhere & IIf(Len(strStudyTabWhere) > 0, "", " AND " & "H.医嘱ID = S.医嘱ID")
        ...
        strSQL = strSQL & vbNewLine & _
                    ...
                    " Where L.Id=M.医嘱ID(+) " & IIf(strSubSql = "", "", " And L.Id=K.Id" & vbNewLine) & _
                            ...
                            IIf(strFilterReportContext = "", "", "     And L.Id In" & strFilterReportContext & vbNewLine) & _
                            IIf(strReportFilter = "", "", strReportFilter & vbNewLine)
        ...
        If mblnMoved Or mblnFindHistory Then
            ...
            '由于处理后Union all 会导致效率降低，故将在线库和历史库分开处理
                ...
                strSQL = "Select /*+driving_site(A) driving_site(B) driving_site(H)*/" & vbNewLine & _
                        ...
                        "         h.随访描述, h.诊断分类, h.检查uid, h.图像位置, a.执行部门id As 执行科室id, 1 As 转出, e.名称 As 病人科室, a.采样时间, c.健康号, c.就诊卡号, a.No As 单据号," & vbNewLine & _
            ...
            If ufgStudyList.GetColIndex("姓名") > 0 Then
                ...
                If lngRowIndex <= 0 And LenB(StrConv(strFilter, vbFromUnicode)) = Len(strFilter) Then
                    For i = intB To lngEndRow
                        If zlCommFun.SpellCode(nvl(ufgStudyList.Text(i, "姓名"), "") & "※0") Like UCase(strFilter) & "*" Then
                            lngRowIndex = i
                            Exit For
                        End If
                    Next i
...
On Error GoTo errhandle
    ...
    Else
        ...
        Else
            ...
            If strCardName = "姓名" And Not mSysPar.blnNameQueryTimeLimit And SQLCondition.姓名 <> "" Then
...
On Error GoTo errhandle
    ...
    If Not mblnIsHistory Then  '---------------------------非历次检查直接用列表中记录填充
        ...
        lbl检查信息.Caption = "病人科室:" & mcurAdviceInf.strPatientDepartment & _
                            "  标识号:" & mcurAdviceInf.strMarkNum & _
    ...
    Else
        If mcurAdviceInf.lngAdviceId > 0 Then
            labStudyNum.Caption = "[" & GetStudyNumberDisplayName & ":" & IIf(mcurAdviceInf.strStudyNum <> "", mcurAdviceInf.strStudyNum, "---") & "]  ●" & mcurAdviceInf.strStuStateDesc
            lbl检查信息.Caption = "病人科室:" & mcurAdviceInf.strPatientDepartment & _
                                  "  标 识 号:" & mcurAdviceInf.strMarkNum & _
...
On Error GoTo errhandle
    ...
    If Not mfrmWork_PacsImg Is Nothing Then
        ...
        Call mfrmWork_PacsImg.zlUpdateOtherInf(IIf(cboTimes.ListCount > 1, True, False), mcurAdviceInf.blnIsTechincalSure, mcurAdviceInf.strDoDoctor, cboTimes)
    End If
    ...
    If Not mobjWork_Report Is Nothing Then
        ...
        If mcurAdviceInf.intStep < 2 And mcurAdviceInf.intStep <> -1 Then
            ...
            Call mobjWork_Report.zlRefreshFace
        Else
            Call mobjWork_Report.zlUpdateAdviceInf(mcurAdviceInf.lngAdviceId, mcurAdviceInf.lngPatId, mcurAdviceInf.lngSendNo, mcurAdviceInf.intStep, mcurAdviceInf.intMoved = 1, mcurAdviceInf.lngBaby)
        End If
    ...
    Select Case TabWindow.Selected.tag
        ...
        Case "报告填写"
            ...
            Call mobjWork_Report.zlUpdateAdviceInf(mcurAdviceInf.lngAdviceId, mcurAdviceInf.lngPatId, _
                mcurAdviceInf.lngSendNo, mcurAdviceInf.intStep, mcurAdviceInf.intMoved, mcurAdviceInf.lngBaby)

            Call mobjWork_Report.zlRefreshFace(blnRefresh, False, True, mobjWork_Report.IsDockActive, mblnRefreshWord)
...
continueBurn:
        ...
        On Error GoTo errFree
            ...
            Set frmBurn = New frmImageBurn
            Call frmBurn.ShowBurn(mlngModule, mlngCur科室ID, lngCurAdviceId, mListAdviceInf.intMoved = 1, Me)
errFree:
            Call Unload(frmBurn)
            Set frmBurn = Nothing
    End If
...
On Error GoTo errH
    ...
    Dim blnRsDataHaveThisData As Boolean '记录集是否存在对应数据
    Dim blnListHaveThisData As Boolean '列表中是否显示对应数据
    Dim blnHaveRs As Boolean '列表中是否显示对应数据
    Dim strFilterOld As String '用于保存之前的ado.filter
    ...
    '如果执行间数据没有变化，终止处理
    If intRowIndex > -1 Then
        blnListHaveThisData = True
        If ufgStudyList.Text(intRowIndex, "执行间") = strRoom Then
            Exit Sub
        End If
    Else
        blnListHaveThisData = False
    End If
    
    blnRsDataHaveThisData = False
    blnHaveRs = False
    blnHaveRs = Not ufgStudyList.AdoData Is Nothing
    If blnHaveRs Then
        If ufgStudyList.AdoData.Filter <> 0 Then strFilterOld = ufgStudyList.AdoData.Filter
        ufgStudyList.AdoData.Filter = ""
        blnHaveRs = blnHaveRs And ufgStudyList.AdoData.RecordCount > 0
    End If

    '判断记录集是否找到对应数据
    If blnHaveRs Then
        Call ufgStudyList.AdoData.MoveFirst
        Call ufgStudyList.AdoData.Find("医嘱ID=" & lngAdviceId)
        If ufgStudyList.AdoData.AbsolutePosition > adPosUnknown Then
'            '记录集存在数据，获取发送号
            blnRsDataHaveThisData = True
            lngSendNo = Val(nvl(ufgStudyList.AdoData!发送号))
            ufgStudyList.AdoData.Filter = strFilterOld
        Else
            blnRsDataHaveThisData = False
        End If
    Else
    ...
    '如果记录集存在数据，更新对应数据。否则从记录集中查询发送号
    If blnRsDataHaveThisData Then
    ...
    If blnListHaveThisData Then
        ufgStudyList.Text(intRowIndex, "执行间") = strRoom
    End If
