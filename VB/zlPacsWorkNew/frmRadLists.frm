...
Private mblnUseInterface As Boolean
...
'写入错误日志
    If Not mblnUseInterface Then Exit Sub
...
On Error GoTo errHandle
    mblnUseInterface = zlDatabase.GetPara(255, glngSys)
...
Private Sub Form_Load()
    ...
    Set mrsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "查询影像检查类别")
    
    Me.lvwKind.ListItems.Clear
    Do While Not mrsTemp.EOF
        Set mobjItem = Me.lvwKind.ListItems.Add(, "_" & mrsTemp!编码, mrsTemp!名称, "kind", "kind")
        mobjItem.SubItems(1) = mrsTemp!编码
        mrsTemp.MoveNext
    Loop
...
Public Sub zlRefItems(Optional lngItemId As Long)
    ...
    Set mrsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "刷新项目列表", CStr(Mid(Me.lvwKind.SelectedItem.Key, 2)))
    
    
    With mrsTemp
        Me.lvwItem.ListItems.Clear
        Do While Not .EOF
            Set mobjItem = Me.lvwItem.ListItems.Add(, "_" & !ID, !名称, "item", "item")
            mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_编码").Index - 1) = !编码
            mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_部位").Index - 1) = IIf(IsNull(!标本部位), "", !标本部位)
            mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_单位").Index - 1) = IIf(IsNull(!计算单位), "", !计算单位)
            Select Case !可行病检
            Case 1
                mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_可行病检").Index - 1) = "1-必须"
            Case 2
                mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_可行病检").Index - 1) = "2-选择进行"
            Case Else
                mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_可行病检").Index - 1) = "0-不可能"
            End Select
            Select Case !可发胶片
            Case 1
                mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_可发胶片").Index - 1) = "1-必须"
            Case 2
                mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_可发胶片").Index - 1) = "2-选择发放"
            Case Else
                mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_可发胶片").Index - 1) = "0-不可能"
            End Select
            mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_报告图象").Index - 1) = IIf(IsNull(!报告图象), "", !报告图象)
            mobjItem.SubItems(Me.lvwItem.ColumnHeaders("_检查准备").Index - 1) = IIf(IsNull(!检查准备), "", !检查准备)
            .MoveNext
        Loop
    End With
