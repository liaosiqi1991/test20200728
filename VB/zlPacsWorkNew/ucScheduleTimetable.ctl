...
Private Sub btnSchLabel_MouseMove(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If mblnDragingLabel = True And Button = 1 Then
        ...
        If mlngBaseX <> X Or mlngBaseY <> Y Then
            Call MoveBtnLabels(Index, mlngBaseX, mlngBaseY, X, Y)
        End If
...
'------------------------------------------------
    ...
    ElseIf lngLeft < vsfTime.ColWidth(0) Then
        '标签在左边超出时间表，需要往上移动一行
        If lngTop <= vsfTime.RowHeight(0) + vsfTime.RowHeight(1) Then
            '标签无法往上继续移动，会从上方超出时间表，停止移动标签
            Exit Function
        Else
            '把所有标签都往上移动一行，按照新的开始和结束时间，重新摆放标签
            lngLeft = vsfTime.Width - (vsfTime.ColWidth(0) - lngLeft)
...
'------------------------------------------------
    ...
    If mblnRestorePos = False Then
        ...
        ElseIf vsfTime.Cell(flexcpData, lngRow, lngCol) = 0 Then
            blnIsOutOfTime = True
            If MsgBox("不在可以预约的时间段内，是否继续预约？", vbYesNo, "检查预约提示") = vbNo Then
                mblnRestorePos = True   '将标签摆放回原来的位置
            End If
        End If
    End If
    ...
    If mblnRestorePos = False Then
        ...
        If mblnRestorePos = False Then
            ...
            iNextPoolIndex = iPoolIndex + 1
            If iNextPoolIndex > UBound(mSchLabelPool) Then iNextPoolIndex = 0
            
            If blnIsOutOfTime = False Then
                '判断预约标签是否在两个号码中间，提示是否自动延长标签
                If mSchLabelPool(iNextPoolIndex).lng序号 - mSchLabelPool(iPrePoolIndex).lng序号 = 2 _
                    And (iPrePoolIndex <> 0 And iNextPoolIndex <> 0) Then
                    If (DateDiff("n", mSchLabelPool(iPrePoolIndex).dtEndTime, mSchLabelPool(iPoolIndex).dtStartTime) = 0 _
                        And DateDiff("n", mSchLabelPool(iPoolIndex).dtEndTime, mSchLabelPool(iNextPoolIndex).dtStartTime) = 0) _
                        Or DateDiff("n", mSchLabelPool(iPrePoolIndex).dtEndTime, mSchLabelPool(iPoolIndex).dtStartTime) > 5 _
                        Or DateDiff("n", mSchLabelPool(iPoolIndex).dtEndTime, mSchLabelPool(iNextPoolIndex).dtStartTime) > 5 Then
                        '不提示
                    Else
                        If MsgBox("这个时间段内，只有一个空序号，是否自动扩展预约标签，填满剩余时间？", vbYesNo, "检查预约提示") = vbYes Then
                            '先修改预约池的开始时间和结束时间，然后再摆放标签
                            mSchLabelPool(iPoolIndex).dtStartTime = mSchLabelPool(iPrePoolIndex).dtEndTime
                            mSchLabelPool(iPoolIndex).dtEndTime = mSchLabelPool(iNextPoolIndex).dtStartTime
                            Call PutSchLabel(iFirstIndex, iPoolIndex)   '直接调整标签位置并退出
                            Exit Function
                        Else
                            '什么都不做
                        End If
                    End If
                ElseIf lngNewNumber = mSchLabelPool(iPrePoolIndex).lng序号 + 1 And iPrePoolIndex <> 0 _
                    And DateDiff("n", mSchLabelPool(iPrePoolIndex).dtEndTime, mSchLabelPool(iPoolIndex).dtStartTime) <= 5 _
                    And DateDiff("n", mSchLabelPool(iPrePoolIndex).dtEndTime, mSchLabelPool(iPoolIndex).dtStartTime) <> 0 Then
                    '如果新序号和上一个号码相连，提示用户是否自动调整预约时间，填满空隙
                    If MsgBox("是否自动调整，将开始时间和上一个预约的结束时间相连？", vbYesNo, "检查预约提示") = vbYes Then
                        '先设置结束时间，再修改开始时间
                        mSchLabelPool(iPoolIndex).dtEndTime = DateAdd("n", DateDiff("n", mSchLabelPool(iPoolIndex).dtStartTime, mSchLabelPool(iPrePoolIndex).dtEndTime), mSchLabelPool(iPoolIndex).dtEndTime)
                        mSchLabelPool(iPoolIndex).dtStartTime = mSchLabelPool(iPrePoolIndex).dtEndTime
                        Call PutSchLabel(iFirstIndex, iPoolIndex)  '直接调整标签位置并退出
                        Exit Function
                    Else
                        '什么都不做
                    End If
                ElseIf lngNewNumber = mSchLabelPool(iNextPoolIndex).lng序号 - 1 And iNextPoolIndex <> 0 _
                    And DateDiff("n", mSchLabelPool(iPoolIndex).dtEndTime, mSchLabelPool(iNextPoolIndex).dtStartTime) <= 5 _
                    And DateDiff("n", mSchLabelPool(iPoolIndex).dtEndTime, mSchLabelPool(iNextPoolIndex).dtStartTime) <> 0 Then
                    '如果新序号和下一个号码相连，提示用户是否自动调整预约时间，填满空隙
                    If MsgBox("是否自动调整，将结束时间和下一个预约的开始时间相连？", vbYesNo, "检查预约提示") = vbYes Then
                        '修改结束时间
                        mSchLabelPool(iPoolIndex).dtStartTime = DateAdd("n", DateDiff("n", mSchLabelPool(iPoolIndex).dtEndTime, mSchLabelPool(iNextPoolIndex).dtStartTime), mSchLabelPool(iPoolIndex).dtStartTime)
                        mSchLabelPool(iPoolIndex).dtEndTime = mSchLabelPool(iNextPoolIndex).dtStartTime
                        Call PutSchLabel(iFirstIndex, iPoolIndex)  '直接调整标签位置并退出
                        Exit Function
                    Else
                    ...
                    End If
    '            ElseIf a Then
    '                '判断标签是否在预约范围的第一个或最后一个，如果是，自动延长预约时间
