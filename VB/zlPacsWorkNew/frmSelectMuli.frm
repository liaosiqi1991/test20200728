                    ...
                    If Inet1.FuncFtpConnect(nvl(rsCurImageData("Host1")), nvl(rsCurImageData("User1")), nvl(rsCurImageData("Pwd1"))) = 0 Then
                        If nvl(rsCurImageData("设备号2")) <> vbNullString Then
                            If Inet2.FuncFtpConnect(nvl(rsCurImageData("Host2")), nvl(rsCurImageData("User2")), nvl(rsCurImageData("Pwd2"))) = 0 Then
                                MsgBoxD Me, "FTP不能正常连接，请检查网络设置。", vbInformation, gstrSysName
                                Exit Sub
                            End If
                        Else
                            MsgBoxD Me, "FTP不能正常连接，请检查网络设置。", vbInformation, gstrSysName
...
On Error GoTo errHandle
    ...
    While Not rsImageDatas.EOF
        ...
        If mlngReleationType = 1 Then
            '当对部分图像取消关联后，临时序列中将保存对应的序列信息，如再次取消关联时，则需要找到该序列，并在此序列下创建图像关联
        ...
        Else
            ...
            strLocalFile = IIf(Len(App.Path) > 3, App.Path & "\TmpImage\", App.Path & "TmpImage\") & nvl(rsImageDatas!Url) & "/" & strImageUID
...
'                            If mlngReleationType = 1 Then Call CancelImageMove(strNewFtpIp, strNewFtpUser, strNewFtpPwd, objMoveList)
                ...
                End If
            ElseIf Len(Dir(strLocalFile)) > 0 Then
            'FTP上不存在而本地存在
                lngResult = objDestFtp.FuncUploadFile(strFtpVirtualPath, strLocalFile, strImageUID)
                
                If lngResult <> 0 Then
                    Call CreateFTPDir(objDestFtp, "", strFtpVirtualPath)
                    lngResult = objDestFtp.FuncUploadFile(strFtpVirtualPath, strLocalFile, strImageUID)
                    If lngResult <> 0 Then
                        Call err.Raise(-1, "MoveImageToStudy", "上传本地关联图像 [" & strLocalFile & "] 到 [" & strFtpVirtualPath & "] 失败，请检查FTP设置。", err.HelpFile, err.HelpContext)
                        Exit Function
                    End If
                End If
                blnIsMove = True
...
errHandle:
    ...
    Call err.Raise(-1, "StartReleation", strErrMsg)  '继续抛出错误
End Function
...
errHandle:
    ...
    Call err.Raise(-1, "CacelReleation", strErrMsg)
End Function
...
Private Sub Form_Unload(Cancel As Integer)
    '保存窗口状态
    Set mdcmUID = Nothing
    Set mrsStudyData = Nothing
    Set mrsSeriesData = Nothing
    Set mrsImageData = Nothing
