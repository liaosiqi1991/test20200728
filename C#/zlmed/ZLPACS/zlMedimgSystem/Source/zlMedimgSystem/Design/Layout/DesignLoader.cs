...
//        #region Constructors /// Empty constructor simply creates a new form.
...
//        }
//        #endregion #region Overriden methods of BasicDesignerLoader // Called by the host when we load a document.
//        protected override void PerformLoad(IDesignerSerializationManager designerSerializationManager)
//        {
//            this.host = this.LoaderHost; if (host == null)
//            {
...
//            string baseClassName; // If no filename was passed in, just create a form and be done with it.  If a file name
//                                  // was passed, read it.
//            if (fileName == null)
//            {
...
//            IComponentChangeService cs = host.GetService(typeof(IComponentChangeService)) as IComponentChangeService; if (cs != null)
//            {
...
//            host.EndLoad(baseClassName, successful, errors); // We've just loaded a document, so you can bet we need to flush changes.
//            dirty = true;
//            unsaved = false;
//        }        /// <summary>
...
//            document.AppendChild(document.CreateElement("DOCUMENT_ELEMENT")); IDesignerHost idh = (IDesignerHost)this.host.GetService(typeof(IDesignerHost));
//            root = idh.RootComponent; Hashtable nametable = new Hashtable(idh.Container.Components.Count);
//            IDesignerSerializationManager manager = host.GetService(typeof(IDesignerSerializationManager)) as IDesignerSerializationManager; document.DocumentElement.AppendChild(WriteObject(document, nametable, root));
...
//            Debug.Assert(value != null, "Should not invoke WriteObject with a null value"); XmlNode node = document.CreateElement("Object");
//            XmlAttribute typeAttr = document.CreateAttribute("type"); typeAttr.Value = value.GetType().AssemblyQualifiedName;
//            node.Attributes.Append(typeAttr); IComponent component = value as IComponent; if (component != null && component.Site != null && component.Site.Name != null)
//            {
//                XmlAttribute nameAttr = document.CreateAttribute("name"); nameAttr.Value = component.Site.Name;
...
//            }
//            bool isControl = (value is Control); if (isControl)
//            {
//                XmlAttribute childAttr = document.CreateAttribute("children"); childAttr.Value = "Controls";
...
//                if (isControl)
//                {
//                    PropertyDescriptor controlProp = properties["Controls"]; if (controlProp != null)
//                    {
//                        PropertyDescriptor[] propArray = new PropertyDescriptor[properties.Count - 1];
//                        int idx = 0; foreach (PropertyDescriptor p in properties)
//                        {
//                            if (p != controlProp)
//                            {
//                                propArray[idx++] = p;
...
//                    }
//                }
//                WriteProperties(document, properties, value, node, "Property"); EventDescriptorCollection events = TypeDescriptor.GetEvents(value, propertyAttributes);
//                IEventBindingService bindings = host.GetService(typeof(IEventBindingService)) as IEventBindingService; if (bindings != null)
//                {
...
//                    XmlAttribute attr = document.CreateAttribute("name"); attr.Value = prop.Name;
//                    node.Attributes.Append(attr); DesignerSerializationVisibilityAttribute visibility = (DesignerSerializationVisibilityAttribute)prop.Attributes[typeof(DesignerSerializationVisibilityAttribute)]; switch (visibility.Visibility)
//                    {
...
//                            object propValue = prop.GetValue(value); if (typeof(IList).IsAssignableFrom(prop.PropertyType))
//                            {
//                                WriteCollection(document, (IList)propValue, node);
//                            }
//                            else
//                            {
//                                PropertyDescriptorCollection props = TypeDescriptor.GetProperties(propValue, propertyAttributes); WriteProperties(document, props, propValue, node, elementName);
//                            }
...
//        {
//            IDesignerHost idh = (IDesignerHost)this.host.GetService(typeof(IDesignerHost)); Debug.Assert(value != null && value.Site != null && value.Site.Container == idh.Container, "Invalid component passed to WriteReference"); XmlNode node = document.CreateElement("Reference");
//            XmlAttribute attr = document.CreateAttribute("name"); attr.Value = value.Site.Name;
//            node.Attributes.Append(attr);
//            return node;
//        }
//        private bool WriteValue(XmlDocument document, object value, XmlNode parent)
//        {
//            IDesignerHost idh = (IDesignerHost)this.host.GetService(typeof(IDesignerHost)); // For empty values, we just return.  This creates an empty node.
//            if (value == null)
//            {
//                return true;
//            }
//            TypeConverter converter = TypeDescriptor.GetConverter(value); if (GetConversionSupported(converter, typeof(string)))
//            {
...
//            {
//                byte[] data = (byte[])converter.ConvertTo(null, CultureInfo.InvariantCulture, value, typeof(byte[])); parent.AppendChild(WriteBinary(document, data));
//            }
//            else if (GetConversionSupported(converter, typeof(InstanceDescriptor)))
//            {
//                InstanceDescriptor id = (InstanceDescriptor)converter.ConvertTo(null, CultureInfo.InvariantCulture, value, typeof(InstanceDescriptor)); parent.AppendChild(WriteInstanceDescriptor(document, id, value));
//            }
...
//                MemoryStream stream = new MemoryStream(); formatter.Serialize(stream, value); XmlNode binaryNode = WriteBinary(document, stream.ToArray()); parent.AppendChild(binaryNode);
//            }
//            else
//            {
//                return false;
...
//                XmlAttribute typeAttr = document.CreateAttribute("type"); typeAttr.Value = obj.GetType().AssemblyQualifiedName;
...
//        {
//            XmlNode node = document.CreateElement("Binary"); node.InnerText = Convert.ToBase64String(value);
//            return node;
//        }
...
//            BinaryFormatter formatter = new BinaryFormatter();
//            MemoryStream stream = new MemoryStream(); formatter.Serialize(stream, desc.MemberInfo); XmlAttribute memberAttr = document.CreateAttribute("member"); memberAttr.Value = Convert.ToBase64String(stream.ToArray());
//            node.Attributes.Append(memberAttr);
//            foreach (object arg in desc.Arguments)
//            {
//                XmlNode argNode = document.CreateElement("Argument"); if (WriteValue(document, arg, argNode))
//                {
...
//            if (!desc.IsComplete)
//            {
//                PropertyDescriptorCollection props = TypeDescriptor.GetProperties(value, propertyAttributes); WriteProperties(document, props, value, node, "Property");
//            }
//            return node;
//        }
//        #endregion #region DeSerialize - Load        /// <summary>
...
//        {
//            string baseClass = null; // Anything unexpected is a fatal error.
//                                     //
//            try
//            {
...
//                string cleandown = sr.ReadToEnd(); cleandown = "<DOCUMENT_ELEMENT>" + cleandown + "</DOCUMENT_ELEMENT>"; XmlDocument doc = new XmlDocument(); doc.LoadXml(cleandown); // Now, walk through the document's elements.
//                                                                                                                                                                                      //
...
//                return;
//            }
//            XmlAttribute nameAttr = childNode.Attributes["name"]; if (nameAttr == null)
//            {
//                errors.Add("No event name");
//                return;
//            }
//            XmlAttribute methodAttr = childNode.Attributes["method"]; if (methodAttr == null || methodAttr.Value == null || methodAttr.Value.Length == 0)
//            {
//                errors.Add(string.Format("Event {0} has no method bound to it"));
//                return;
//            }
//            EventDescriptor evt = TypeDescriptor.GetEvents(instance)[nameAttr.Value]; if (evt == null)
//            {
//                errors.Add(string.Format("Event {0} does not exist on {1}", nameAttr.Value, instance.GetType().FullName));
//                return;
//            }
//            PropertyDescriptor prop = bindings.GetEventProperty(evt); Debug.Assert(prop != null, "Bad event binding service");
//            try
//            {
...
//            //
//            XmlAttribute memberAttr = node.Attributes["member"]; if (memberAttr == null)
//            {
...
//            object[] args = null; // Check to see if this member needs arguments.  If so, gather
//                                  // them from the XML.
//            if (mi is MethodBase)
//            {
//                ParameterInfo[] paramInfos = ((MethodBase)mi).GetParameters(); args = new object[paramInfos.Length]; int idx = 0; foreach (XmlNode child in node.ChildNodes)
//                {
//                    if (child.Name.Equals("Argument"))
//                    {
//                        object value; if (!ReadValue(child, TypeDescriptor.GetConverter(paramInfos[idx].ParameterType), errors, out value))
//                        {
//                            return null;
//                        }
//                        args[idx++] = value;
...
//            object instance = id.Invoke(); // Ok, we have our object.  Now, check to see if there are any properties, and if there are, 
//                                           // set them.
//                                           //
...
//        {
//            XmlAttribute typeAttr = node.Attributes["type"]; if (typeAttr == null)
//            {
//                errors.Add("<Object> tag is missing required type attribute");
//                return null;
//            }
//            Type type = Type.GetType(typeAttr.Value); if (type == null)
//            {
...
//            object instance; if (typeof(IComponent).IsAssignableFrom(type))
//            {
//                if (nameAttr == null)
//                {
...
//            IList childList = null; if (childAttr != null)
//            {
//                PropertyDescriptor childProp = TypeDescriptor.GetProperties(instance)[childAttr.Value]; if (childProp == null)
//                {
...
//                    if (childList != null)
//                    {
//                        object childInstance = ReadObject(childNode, errors); childList.Add(childInstance);
//                    }
//                }
...
//        {
//            XmlAttribute nameAttr = node.Attributes["name"]; if (nameAttr == null)
//            {
//                errors.Add("Property has no name");
//                return;
//            }
//            PropertyDescriptor prop = TypeDescriptor.GetProperties(instance)[nameAttr.Value]; if (prop == null)
//            {
...
//              // 3.  A collection.
//              //
//            bool isContent = prop.Attributes.Contains(DesignerSerializationVisibilityAttribute.Content); if (isContent)
//            {
//                object value = prop.GetValue(instance); // Handle the case of a content property that is a collection.
//                                                        //
//                if (value is IList)
//                {
...
//                        {
//                            object item;
//                            XmlAttribute typeAttr = child.Attributes["type"]; if (typeAttr == null)
//                            {
...
//            else
//            {
//                object value; if (ReadValue(node, prop.Converter, errors, out value))
//                {
...
//                    {
//                        byte[] data = Convert.FromBase64String(child.InnerText); // Binary blob.  Now, check to see if the type converter
...
//                        {
//                            BinaryFormatter formatter = new BinaryFormatter();
//                            MemoryStream stream = new MemoryStream(data); value = formatter.Deserialize(stream);
//                            return true;
//                        }
//                    }
...
//            StringWriter sw;
//            sw = new StringWriter(); XmlTextWriter xtw = new XmlTextWriter(sw); xtw.Formatting = Formatting.Indented;
//            xmlDocument.WriteTo(xtw); string cleanup = sw.ToString().Replace("<DOCUMENT_ELEMENT>", ""); cleanup = cleanup.Replace("</DOCUMENT_ELEMENT>", "");
//            sw.Close();
//            return cleanup;
//        }
//        public void Save()
...
//            try
//            {
//                Flush(); int filterIndex = 3; if ((fileName == null) || forceFilePrompt)
//                {
//                    SaveFileDialog dlg = new SaveFileDialog(); dlg.DefaultExt = "xml";
...
//                                XmlTextWriter xtw = new XmlTextWriter(sw); xtw.Formatting = Formatting.Indented;
//                                if (xmlDocument == null) xmlDocument = new XmlDocument();
//                                xmlDocument.WriteTo(xtw); // Get rid of our artificial super-root before we save out
//                                                          // the XML.
//                                                          //
//                                string cleanup = sw.ToString().Replace("<DOCUMENT_ELEMENT>", ""); cleanup = cleanup.Replace("</DOCUMENT_ELEMENT>", "");
//                                xtw.Close(); StreamWriter file = new StreamWriter(fileName); file.Write(cleanup);
//                                file.Close();
//                            }
//                            break;
//                    }
